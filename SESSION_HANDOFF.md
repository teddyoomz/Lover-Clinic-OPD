...
- **NEW (2026-07-21) — worry-list sweep 12 + fx-perf + hub pagination + iOS wedge-PREVENTION — ALL DEPLOYED LIVE (vercel-only ×5, rules UNCHANGED)**: master `2e343c89` = prod (`lover-clinic-e2q4c5j5p` aliased 200; new code verified IN live bundle). FULL vitest **18,187/18,187 · 0 fail** + build clean. **① whole-app 5-dim audit → 12 fixes** (Workflow 5-agent, adjudicated first-hand): 💣 Rule M backfill 3 legacy inline-blob be_treatments (1021KB=100%cap→5KB) · FB webhook fail-CLOSED (prove-red สด: unsigned POST 200→401 บน LIVE) · backup partial-warn + LINE-reminder failed-night warn (เคยเขียวหลอก ok:true) · tfp-options checkRevoked + line-reminder crons fail-closed CRON_SECRET · dead-man's switch HEALTHCHECK_PING_URL · staleness banner >36h 2 shells · retention keep-last-valid guard (V122 streak จะไม่ลบ backup valid สุดท้าย) · QR self-host `<QrImage>` เลิก qrserver.com 4 จุด (L1+Q-vis) · heartbeat dedup 144→2 docs/วัน (chart+opd sweep per-day doc) · off-site backup pull (F:\LoverClinic-backups auto-20260720 verified 79col/hash) + docs/RUNBOOK-restore.md · npm uninstall cheerio+bottleneck (undici HIGH หลุด prod tree). universal classifiers SH2-4/BW4/FP4 กัน drift. 07:35 sweep รอบแรก 14/14 เขียว alert ไม่ยิง (ถูก). **② fx-perf** (iOS จอขาวเลื่อน): 10 การ์ด TFP breathe box-shadow infinite → หน้า invalidate 60fps → tile ไม่ cache; fix `--fx-anim` pause plane 19 rules → html.is-scrolling หยุดตอนเลื่อน (ทุก tier) + eco tier (fast=full; measured jank/hw floor) + health-card toggle. **③ hub pagination** 20/หน้าทุก tab (past-30 270→20 การ์ด, DOM 21,359→3,000 L1 LIVE); RENDER-side (query แชร์ badge). **④ iOS wedge-PREVENTION** (systematic-debugging, beacon-proven 13:23:03 wedge→:09 reload→:22 wedge, POST สำเร็จ=เน็ตดี, Windows ก็โดน): root = AV214 ตันที่ reload-config-เดิม, IDB/lease=origin storage รอด reload → วนไม่จบ; AV212 escape เอื้อมไม่ถึง. **3 rung**: bootCacheWatchdog (cache-read แข่ง 3s → ค้าง=memory-cache+reload ตอนหน้าโหลด="ครั้งแรกไม่เจอ"; false-positive guarded 4 เคส + 10min cooldown) · escalateWedgeIfReloadFailed (gated ด้วย reachability probe — ปิดแคชเฉพาะเน็ต Firestore พิสูจน์ว่าปกติ) · hub spinner bound 10s. reason `conn-wedge`(24h)≠`idb-slow`(14d) — ห้ามเรียกเครื่องเร็วว่าช้า (user: "17 pro max ไม่ช้า"). L1 จับ dead-feature ใน commit เอง (probe path string≠segment form → no-op เงียบ) → getLastBootWatchdogVerdict() พิสูจน์รันจริง. ~14 commits · ~150 เทสใหม่. Checkpoint `.agents/sessions/2026-07-21-worrylist-perf-wedge.md`.
...
### Session 2026-07-21 — worry-list 12 + fx-perf + hub pagination + iOS wedge-PREVENTION — DEPLOYED

master `2e343c89` = prod (`lover-clinic-e2q4c5j5p` aliased 200; rules UNCHANGED → vercel-only ×5). FULL vitest **18,187/0** + build clean. code backlog = ศูนย์.
- **worry-list (5-dim audit → 12 fixes)**: 💣 backfill 3 legacy 1MB-cap treatments · FB webhook fail-closed (prove-red 200→401) · backup/LINE warn contract · dead-man's switch · retention guard · QR self-host · heartbeat dedup 144→2/วัน · off-site backup + runbook · dead-dep removal. 07:35 sweep 14/14 เขียว.
- **fx-perf**: iOS จอขาว root (10 การ์ด breathe box-shadow infinite → invalidate 60fps) → `--fx-anim` pause plane 19 rules + scroll-pause + full/eco tier.
- **hub pagination**: 270→20 การ์ด/หน้าทุก tab (DOM 21,359→3,000, L1 LIVE data จริง; RENDER-side).
- **iOS wedge-prevention** (beacon-proven, systematic-debugging): AV214 ตันที่ reload-config-เดิม (IDB/lease=origin storage). 3 rung: bootCacheWatchdog (3s cache-race→memory-cache reload="ครั้งแรกไม่เจอ") + escalation gated ด้วย reachability probe + spinner bound 10s. reason conn-wedge(24h)≠idb-slow(14d) — ห้ามเรียกเครื่องเร็วว่าช้า.
- L1 จับ dead-feature ใน commit เอง (probe string≠segment path→no-op เงียบ). Honest gap: wedge จริงจำลองนอกหน้าเว็บไม่ได้ → timeout branch พิสูจน์ด้วย injection; ตัวจริง = user iPhone L1.
- Checkpoint `.agents/sessions/2026-07-21-worrylist-perf-wedge.md`.
...
📂 **Older sessions (`2026-07-07 EOD+2` and earlier) + older Current-State index entries → `.agents/sessions/session-handoff-archive.md`** (cold storage, NOT read at boot).
