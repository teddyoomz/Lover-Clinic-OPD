...
- **Date last updated**: 2026-05-21 EOD+1 LATE — Tablet Chart more-tools shipped + **post-ship CRITICAL canvas-init bug FIXED** (user on-device L1 caught it; /systematic-debugging); ratio fix DEPLOYED earlier
- **Master**: `b638fe9d` (clean, pushed). **Prod**: `d750c725` — ratio fix `72ea7585` LIVE. **more-tools + init-once fix = 13 commits ahead, NOT deployed.** Storage CORS live (bucket-side). V106 + V108 also live.
- **Tests**: full vitest **GREEN (13927/0)** · build clean (~3s). +tablet-chart-more-tools (U1-U4 + R1 + F1-F2 + **RC1-RC3 root-cause**) + L2 e2e 9/0 real prod + AV103.
