...
- **Date last updated**: 2026-05-21 EOD+1 LATE+3 — Tablet Chart more-tools: LIVE-DISPLAY bug REAL on-device cause FOUND + FIXED (fix4) — an inline `background:#fff` on the canvas → Fabric copies it to the opaque **upper-canvas** → COVERED the painted lower-canvas (blank live + correct save). Proven in a real browser (isolation) + on-device DIAG. Fix = remove inline bg (mirror ChartCanvas). Awaiting on-device re-confirm + deploy.
- **Master**: clean, pushed. **Prod**: `d750c725` — ratio fix LIVE. **more-tools + 4 post-ship fixes (init-once + save + sync-render + upper-canvas-cover) = ~22 commits ahead, NOT deployed.** Storage CORS live. V106 + V108 also live.
- **Tests**: full vitest **GREEN** · build clean · **fix4 proven in a real browser** (fabric-reexport isolation: WITH inline canvas bg → upper-canvas computed white = opaque cover; WITHOUT → transparent) + **on-device DIAG screenshot** (`paint c7 w42 t0` = lower-canvas IS painted; screen white = covered; `raf/s 145` = rAF alive on the user's real browser → fix3's rAF theory was a headless-preview artifact). +RC1-RC11 + AV103/AV104/AV105. Rule S (Chrome MCP / real-browser standing auth) added.
- **Deploy**: combined (user-triggered, V18) — `vercel --prod` (more-tools + all 4 post-ship fixes; ~22 commits) **+** `firebase deploy --only storage` (the fix2 storage.rules json allowance → enables lossless re-edit; **Probe-Deploy-Probe #13**: anon write `uploads/chart-edit-sessions/...` → 403, staff json → 200). [⚠ firebase CLI 15.x: `--only storage`, NOT `storage:rules`.] Then user on-device re-confirm (live render + every tool + save→PC).
