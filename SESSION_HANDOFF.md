...
- **Date last updated**: 2026-05-21 EOD+1 LATE+2 — Tablet Chart more-tools: LIVE-DISPLAY render bug FIXED (rAF-deferred render → blank live canvas; sync `renderAll` fix), pixel-verified in a real browser @ forced dpr=2. ALL post-ship symptoms (init + save + live-render) now resolved; storage.rules json still awaits deploy.
- **Master**: clean, pushed. **Prod**: `d750c725` — ratio fix LIVE. **more-tools + 3 post-ship fixes (init-once + save + sync-render) = ~18 commits ahead, NOT deployed.** Storage CORS live. V106 + V108 also live.
- **Tests**: full vitest **GREEN (13932/0)** · build clean · **render fix pixel-verified** (`getImageData` on the live canvas in a real browser @ forced `config.devicePixelRatio=2` → template paints + stroke paints live, rAF dead) · full-relay Playwright e2e GREEN on real prod. +RC1-RC8 + AV103/AV104.
- **Deploy**: combined (user-triggered, V18) — `vercel --prod` (more-tools + all 3 post-ship fixes; ~18 commits) **+** `firebase deploy --only storage` (the fix2 storage.rules json allowance → enables lossless re-edit; **Probe-Deploy-Probe #13**: anon write `uploads/chart-edit-sessions/...` → 403, staff json → 200). [⚠ firebase CLI 15.x: `--only storage`, NOT `storage:rules`.] Then user on-device iPad L1 (live render + every tool + save→PC).
