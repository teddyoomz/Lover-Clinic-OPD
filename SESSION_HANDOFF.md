...
- **Date last updated**: 2026-05-21 EOD+2 — **RE-EDIT a saved chart ON TABLET — IMPLEMENTED + verified (local, NOT deployed).** edit ✏️ on a saved chart → PcPairingModal (reuses the add-new PC/tablet choice); send-to-tablet ships the existing chart PNG (`templateImageUrl` raster fallback) + `fabricJson` (NEW session field `editFabricJsonUrl` via the existing `uploadTransportJson('edit')`); `TabletChartEditorPage` resolves json-FIRST → `TabletChartCanvas` NEW `initialFabricJson` prop → object-level `loadFromJSON` at saved dims (mirror PC ChartCanvas) else raster `loadTemplate` (template effect early-returns → no double-load); result merges back to the SAME slot (`editingIdx` preserved through the relay). **Reuses serializeFabricCanvas/isObjectLevelReeditable/uploadTransportJson — no new collection, no new storage rule.** Object-level tablet path is **live-gated on the storage deploy** (#13; pre-deploy = raster fallback, works).
...
- **Master**: clean, pushed (re-edit-on-tablet feature). **Prod**: `d750c725` — ratio fix LIVE. **more-tools + 5 post-ship rounds + re-edit-on-tablet = ~25 commits ahead, NOT deployed.** Storage CORS live. V106 + V108 also live.
- **Tests**: full vitest **GREEN (13965/0)** · build clean · **L2 e2e ALL PASS on real prod** (`scripts/e2e-chart-relay-roundtrip.mjs` — A/B/**E new re-edit-on-tablet leg** (editFabricJsonUrl + PRODUCTION isObjectLevelReeditable on the round-tripped edit.json + dims survive + same-slot merge) /C1-C4 stress) · **L1 real-browser object-level hydrate confirmed** (mounted REAL TabletChartCanvas with real-fabric initialFabricJson → exportObjects 2, exportW/H 600×800 native, NOT container-fit raster) · `tests/re-edit-chart-on-tablet.test.jsx` RT1-RT7 + RC2/R4.2/R4.4 V21 fixups.
- **Deploy**: combined (user-triggered, V18) — `vercel --prod` (more-tools + all 5 post-ship rounds; ~24 commits) **+** `firebase deploy --only storage` (storage.rules json → unlocks **live object-level re-edit**; **Probe-Deploy-Probe #13**: anon write `uploads/chart-edit-sessions/...` → 403, staff json → 200). [⚠ firebase CLI 15.x: `--only storage`, NOT `storage:rules`.] Then on-device: tablet edit → save → PC re-edit shows prior strokes as MOVABLE objects.
