...
**Pick-at-treatment late-visit persistence (2026-04-24, after "ไม่แสดงรายการอะไรเลย" bug):**
- Root cause: `options.customerCourses` filter `[].every()` = `true` → placeholder with empty `products: []` dropped by `allZero` guard → "เลือกสินค้า" button never rendered. Also `assignCourseToCustomer` didn't special-case `เลือกสินค้าตามจริง` → SaleTab-bought pick courses wrote N per-product entries instead of ONE placeholder, masking the bug with wrong shape.
- `src/components/TreatmentFormPage.jsx` — filter exempts `isPickAtTreatment && needsPickSelection` from `allZero` drop; `customerCoursesForForm` detects persisted placeholder (`c.needsPickSelection && Array.isArray(c.availableProducts)`) and re-emits placeholder shape with `_beCourseId` + `_beCourseIndex` for pick persistence; PickProductsModal onConfirm calls `resolvePickedCourseInCustomer` when `saveTarget === 'backend'` + `customerId` + `_beCourseId/_beCourseIndex` present.
- `src/lib/backendClient.js` — `assignCourseToCustomer` branch for `'เลือกสินค้าตามจริง'` writes ONE placeholder with stable `courseId: pick-${Date.now()}-${hex}` + `needsPickSelection: true` + `availableProducts: [...]` (options carry productId/qty/unit/min/max); NEW `resolvePickedCourseInCustomer(customerId, courseKey, picks)` splice-replaces placeholder with N resolved per-product entries. `courseKey` accepts string (persistent courseId — survives splice-shift) OR number (legacy index fallback).
- `src/components/backend/CustomerDetailView.jsx` — `activeCourses` filter exempts `c.needsPickSelection` so placeholder stays in "คอร์สของฉัน" active tab; renders teal "เลือกสินค้าเพื่อใช้ (N ตัวเลือก)" badge + "เลือกในหน้าสร้างการรักษา" hint.
- Tests: `tests/phase12.2b-scenarios.test.js` Scenario 21 +10 (S21.1-S21.10) — pure helper coverage + 6 source-grep invariants (S21.3 assignCourseToCustomer branch, S21.4 resolvePickedCourseInCustomer contract, S21.5 customerCoursesForForm mapper, S21.6 TreatmentForm filter exemption, S21.7 CustomerDetailView filter + badge, S21.8 onConfirm persist wiring). `tests/phase12-catalog-tabs.test.jsx` RE7 slice-window bumped 2500→4500 (new branch pushed sentinel past old window).

**In-visit pick + use same-treatment (2026-04-24, after "คอร์สคงเหลือไม่พอ: LipoS ต้องการตัด 1 เหลือตัดไม่ได้อีก 1" bug — fix required TWO passes):**
- **Pass 1 root cause**: `TreatmentFormPage.handleSubmit` iterated `purchasedItems` and passed `course.products` (master options list) to `assignCourseToCustomer` with `courseType: 'เลือกสินค้าตามจริง'` → my placeholder branch fired AGAIN → the doctor's in-visit picks (resolved in `options.customerCourses` via PickProductsModal) were overwritten with a fresh placeholder on save → `deductCourseItems` couldn't find the picked product → "คอร์สคงเหลือไม่พอ". User symptom: bought แฟต 4 เข็ม, picked LipoS 4/4, ticked + qty 1 → save → error despite UI showing 4/4 available.
- **Pass 2 root cause (same symptom, deeper bug)**: Even after `alreadyResolved` flag was wired, the error persisted. `resolvePickedCourseEntry` stamps sub-row rowIds with prefix `picked-purchased-course-...`. Four filter sites in `handleSubmit` used only `purchased-|promo-` prefix checks → picked rows leaked into Phase-1 (pre-auto-sale) deduction bucket → `deductCourseItems` fired on a `customer.courses` that didn't yet have LipoS (auto-sale hadn't run) → same throw. Tests that covered helper output in isolation missed this — only the full buy→pick→save→filter chain exercises the prefix routing.
- `src/lib/treatmentBuyHelpers.js` — NEW `resolvePurchasedCourseForAssign(course, customerCourses, purchasedQty)`. For `เลือกสินค้าตามจริง`, looks up the RESOLVED entry in `customerCourses` (by `isAddon && purchasedItemId === course.id && needsPickSelection === false`) and returns `{products: resolvedPicks, alreadyResolved: true}`. For other course types OR when the doctor hasn't picked yet, returns `{products: masterOptions, alreadyResolved: false}`.
- `src/lib/backendClient.js` — `assignCourseToCustomer` placeholder branch gates on `!masterCourse.alreadyResolved` so in-visit resolved picks bypass the placeholder write and land as standard per-product entries. Late-visit path (SaleTab buy → no pick yet) still writes placeholder.
- `src/components/TreatmentFormPage.jsx` — both `handleSubmit` call sites (create-path line ~2245 + edit→sale-path line ~2371) use the helper + forward `alreadyResolved` into `assignCourseToCustomer`.
- NEW `isPurchasedSessionRowId(rowId)` in `treatmentBuyHelpers.js` — classifies `purchased-*|promo-*|picked-*` as purchased-this-visit. All FIVE filter sites in `TreatmentFormPage.handleSubmit` (pre-validation `isPurchased`, Phase-1 `existingDeductions`, edit-mode `oldExisting`/`oldPurchased`, Phase-2 `purchasedDeductions`) now use the helper instead of raw `rowId?.startsWith('purchased-|promo-')` checks. Rationale: `picked-` prefix must be classified as session-purchased so Phase-1 skips it (customer.courses doesn't yet have the picked product — auto-sale assigns it in the SAME handleSubmit).
- Tests: Scenario 21 +10 total (S21.11-S21.20) — S21.17 asserts resolved rowId starts with `picked-` (contract with save filter), S21.18 prefix-classification table, S21.19 full-chain end-to-end (buy→pick→build courseItems→filter split→confirm Phase-1 empty + Phase-2 has LipoS), S21.20 regression guard scanning TreatmentFormPage for leftover raw `startsWith('purchased-|promo-')` (fails if a future refactor adds a new filter site without using the helper).

**Buffet course (บุฟเฟต์) + blood-type dropdown (2026-04-25):**
- **Bug 1 (buffet display wrong)**: "NA- Botox บุฟเฟ่" rendered sub-products as "1/1 U" (specific-qty style) instead of "บุฟเฟต์" text. Per ProClinic (user's image 2/3): buffet = unlimited use until date-expiry, qty column shows Thai text "บุฟเฟต์", stock decrements normally on each use but the course stays in "กำลังใช้งาน" forever.
- **Bug 2 (blood-type dropdown empty)**: `bloodTypeOptions: ['A','B','AB','O','ไม่ทราบ']` was a string array but render at line 2740 mapped `b.id` + `b.name` (undefined on strings) → dropdown rendered `<option key=undefined value=undefined>` → empty. Fix: `.map(v => ({ id: v, name: v }))`.
- Files touched:
  - `src/lib/treatmentBuyHelpers.js` — `buildPurchasedCourseEntry` stamps `isBuffet: true` on course + product rows when courseType === 'บุฟเฟต์'. NEW `mapRawCoursesToForm(rawCourses)` helper extracted from TFP inline logic so the 4-branch mapper (pick-at-treatment placeholder / เหมาตามจริง / บุฟเฟต์ / specific-qty) is unit-testable. Buffet exempt from the `total>0 && remaining<=0` consumed-skip so it stays in list forever.
  - `src/components/TreatmentFormPage.jsx` — `customerCoursesForForm` uses the new `mapRawCoursesToForm` helper (6 files → 1 helper). Course column render: `product.isBuffet` → shows violet italic "บุฟเฟต์" (alongside fillLater's "เหมาตามจริง" and specific-qty's "X / Y unit"). Course header badge: "(บุฟเฟต์)" violet badge alongside "(ระบุตอนรักษา)" / "(ซื้อเพิ่ม)". customerCourses filter exempts buffet from allZero drop. Pre-validation skips buffet (liveIsBuffet / inMemoryIsBuffet continues the loop). Product's `exhausted` flag excludes buffet from disabled-checkbox. bloodTypeOptions mapped to objects.
  - `src/lib/backendClient.js` — `deductCourseItems` Step-1 `courseType === 'บุฟเฟต์'` → `continue` (no-op, no qty change). Step-2a fallback matches buffet → `remaining = 0; break` (consumed without touching qty). Step-2b scan skips buffet to avoid double-handling.
  - `src/components/backend/CustomerDetailView.jsx` — `activeCourses` filter exempts buffet courses (unlimited until date-expiry, parseQtyString on stored sentinel could parse to 0 after heavy use). `CourseItemBar` renders violet "บุฟเฟต์" in qty column + full-width violet progress bar (same tone as fill-later, both are "stock-doesn't-decrement" course concepts).
- Tests: `tests/phase12.2b-flow-simulate.test.js` NEW file, +94 tests across 15 describe blocks (F1 rowId prefix contract, F2 mapRawCoursesToForm branches 12 tests incl buffet, F3-F9 end-to-end A1-A7 matrix, F10 buffet end-to-end 10 tests, F11 bloodType objects, F12 filter chain mixed carts, F13 adversarial edge cases, F14 lifecycle integrity, F15 source-grep regression guards). Mirror helpers `filterCustomerCourses` / `preValidateCourseDeductions` / `buildCourseItems` / `splitDeductions` / `simulateDeductCourseItems` replicate the inline TFP logic so scenarios chain end-to-end without React mount.
- Updated: `tests/phase12-catalog-tabs.test.jsx` RE7 re-pointed from TFP inline to mapRawCoursesToForm helper. `tests/phase12.2b-scenarios.test.js` S21.5 same re-point, S21.6 slice window widened 400→900 (buffet exemption landed between pick-at-treatment exemption and allZero line).
