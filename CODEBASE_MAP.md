...
> อัพเดทล่าสุด: 2026-03-28 (Deposit booking system, service completed flow, ProClinic deposit sync)
...
└── pages/
    ...
    ├── AdminDashboard.jsx      — Admin main page (queue, history, deposit, settings nav)
...
| `adminMode` | 41 | 'dashboard' / 'formBuilder' / 'clinicSettings' / 'history' / 'deposit' / 'depositHistory' |
...
| `handleDepositSync(session)` | — | Two-step: fillProClinic (Step 1) → submitDeposit (Step 2), saves depositProClinicId |
| `handleDepositCancel(session)` | — | Cancel deposit in ProClinic + delete customer → archive session |
...
│   │   └── หน้าคิว (badge) | จองมัดจำ (badge) | จัดการ | ตั้งค่า | ประวัติ | ประวัติจอง
│   └── Desktop nav (hidden md:flex)
│       └── หน้าคิว (badge) | จองมัดจำ (badge) | จัดการแบบฟอร์ม | ตั้งค่า | ประวัติ | ประวัติจอง | + สร้างคิว | 🔔 | logout
...
```
cancelDeposit(proClinicId, proClinicHN) → cancel deposit + delete customer in ProClinic → archive
```
