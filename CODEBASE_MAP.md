...
| `adminMode` | 41 | 'dashboard' / 'formBuilder' / 'clinicSettings' / 'history' / 'deposit' / 'depositHistory' / 'appointment' |
...
│   │   └── หน้าคิว (badge) | จอง (badge) | ประวัติ | จัดการ | ตั้งค่า
│   └── Desktop nav (hidden md:flex)
│       └── หน้าคิว (badge) | จองมัดจำ (badge) | ประวัติ | จัดการแบบฟอร์ม | ตั้งค่า | + สร้างคิว | 🔔 | logout
...
├── adminMode === 'appointment' → ProClinic appointment calendar (month view + day detail)
...
### Data
- อ่านจาก Firestore: `clinic_schedules/{token}` (snapshot จากตอน Gen)
- Fields: `months`, `doctorDays`, `closedDays`, `bookedSlots`, `clinicOpenTime`, `clinicCloseTime`, `slotDurationMins`

### Features
- Calendar view แบ่งตามเดือน (month tabs)
- วันหมอเข้า (sky highlight) / วันปิด (red/disabled) / วันว่าง (green badge)
- กดวันที่ → แสดง time slots (ว่าง/ไม่ว่าง) — คำนวณจาก clinic hours ลบ bookedSlots
...
### State (บรรทัด ~170-180)
| State | ใช้ทำอะไร |
|-------|-----------|
| `showScheduleModal` | เปิด/ปิด modal สร้างลิงก์ |
| `schedStartMonth` | เดือนเริ่มต้น (YYYY-MM) |
| `schedAdvanceMonths` | จำนวนเดือนล่วงหน้า (0-3) |
| `schedDoctorDays` | Set ของวันที่หมอเข้า |
| `schedClosedDays` | Set ของวันปิด |
...
2. Modal: เลือกเดือน + ล่วงหน้า + toggle วันหมอเข้า/ปิด (cycle: ปกติ→หมอเข้า→ปิด)
3. กด "Sync + สร้างลิงก์" → sync appointments ทุกเดือน → สร้าง clinic_schedules/{token}
4. แสดง QR + URL ให้ copy
