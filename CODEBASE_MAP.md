...
> อัพเดทล่าสุด: 2026-04-08 (Backend Dashboard Phase 1, Appointment Manager CRUD, Treatment Records Phase 2, Master Data Sync Phase 1, Chat system FB/LINE, Import from ProClinic)
...
│   ├── TreatmentFormPage.jsx   — Full-page treatment create form: mirrors ProClinic layout (Phase 2). saveTarget prop: 'proclinic'|'backend'
│   ├── BackendSyncPanel.jsx    — Manual sync master data (Products, Courses, Doctors, Staff) from ProClinic → be_* collections
│   ├── BackendCustomerSearch.jsx — Search customers from ProClinic → clone to be_customers → card view with detail/treatment buttons
│   └── PrintTemplates.jsx      — OfficialOPDPrint + DashboardOPDPrint components
├── lib/
│   ├── brokerClient.js         — API client for /api/proclinic/* endpoints
│   └── backendClient.js        — Firestore CRUD for backend collections (be_*): saveMasterData, saveCustomer, saveBackendTreatment, etc.
└── pages/
    ├── AdminLogin.jsx          — Login page
    ├── AdminDashboard.jsx      — Admin main page (queue, history, deposit, appointment, backend, settings nav)
    ├── BackendDashboard.jsx    — ระบบหลังบ้าน: tabs (Sync/Search), CustomerDetailOverlay, wires to TreatmentFormPage
...
├── pc_customer_appointments/{customerId} — Appointment list per customer (synced via appointment.js listByCustomer)
├── form_templates/{id}         — Custom form templates
├── be_master_products/{id}     — Backend: synced products (ยา/บริการ/สินค้า) from ProClinic
├── be_master_courses/{id}      — Backend: synced course catalog
├── be_master_doctors/{id}      — Backend: synced doctors/assistants
├── be_master_staff/{id}        — Backend: synced staff
├── be_sync_status/master       — Backend: sync metadata {products/courses/doctors/staff: {lastSyncedAt, count}}
├── be_customers/{proClinicId}  — Backend: cloned customer data (patient shape = reverseMapPatient output)
└── be_treatments/{autoId}      — Backend: treatment records (same shape as treatments/ backup, syncedToProClinic: false)
```
...
| `adminMode` | 41 | 'dashboard' / 'formBuilder' / 'clinicSettings' / 'history' / 'deposit' / 'depositHistory' / 'appointment' / 'chat' / 'backend' |
...
| `apptSearchQuery` | ~282 | search query สำหรับค้นลูกค้าในส่วนจัดการนัดหมาย |
| `apptSearchResults` | ~283 | ผลค้นหาลูกค้า (array \| null) |
| `apptSearching` | ~284 | กำลังค้นหาอยู่ |
| `apptSelectedCustomer` | ~285 | ลูกค้าที่เลือก { id, name, hn, phone } |
| `apptCustomerAppts` | ~286 | นัดหมายของลูกค้าที่เลือก (array from listCustomerAppointments) |
| `apptCustomerLoading` | ~287 | กำลังโหลดนัดหมายของลูกค้า |
| `apptFormMode` | ~288 | null \| { mode: 'create'\|'edit', appointmentId? } — เปิด/ปิด form สร้าง/แก้ไขนัดหมาย |
| `apptFormData` | ~289 | { date, startTime, endTime, doctor, advisor, room, source, appointmentTo, note } |
| `apptFormSaving` | ~290 | กำลังบันทึกนัดหมาย |
...
| `handleApptSearch()` | ~559 | ค้นหาลูกค้าจาก ProClinic (via broker.searchCustomers) สำหรับจัดการนัดหมาย |
| `handleApptSelectCustomer(customer)` | ~575 | เลือกลูกค้า → โหลดนัดหมายทั้งหมด (via broker.listCustomerAppointments) |
| `handleApptEdit(appt)` | ~594 | เปิด form แก้ไขนัดหมาย pre-filled จาก appointment data |
| `handleApptFormSubmit()` | ~604 | สร้าง/แก้ไขนัดหมาย → broker.createAppointment / broker.updateAppointment + re-fetch |
| `handleApptDelete(appointmentId)` | ~656 | ลบนัดหมาย → broker.deleteAppointment + re-fetch |
...
├── adminMode === 'appointment' → ProClinic appointment calendar (month view + slot duration selector + "ว่าง X" per day + day detail + doctor hour drag column sky/blue) + Appointment Manager section (~line 3983): search customer → view/create/edit/delete appointments with real-time refresh
├── adminMode === 'backend' → <BackendDashboard> (ระบบหลังบ้าน: Sync master data + Customer search/detail + Treatment save to backend)
...
// Appointment Manager (appointment.js)
createAppointment(appointment)                         // POST create appointment in ProClinic
updateAppointment(appointmentId, appointment)          // POST update appointment (POST + _method=PUT)
deleteAppointment(appointmentId)                       // POST delete appointment (POST + _method=delete)
listCustomerAppointments(customerId)                   // POST list all appointments for customer (FullCalendar range query)
// Master Data Sync
