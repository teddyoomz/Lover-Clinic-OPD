...
**Rule H** (`.Codex/rules/00-session-start.md`): ALL master data lives in OUR Firestore; ProClinic sync = seed-only; every master-data entity gets a CRUD UI backed by `be_*`; ProClinic-origin ids (products/staff/doctors/courses) stay stable but categorization/units/instruments/holidays/branches/permissions are OWNED locally.
