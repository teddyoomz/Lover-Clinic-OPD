...
**Grep**: `StockBalancePanel.jsx` `load()` must call `listStockBatches({ branchId: locationId })` (NO status) + filter `b.status === 'active' || b.status === 'depleted'`; must NOT call `listStockBatches({ branchId: locationId, status: 'active' })`. **Sanctioned exclusions** (active-only, correct): adjust/transfer/withdrawal batch pickers. **Cross-link**: `tests/v143-stock-balance-show-depleted.test.js` · diag `scripts/diag-nakhon-stock-state.mjs`.
