...
**Grep**: `TreatmentFormPage.jsx` must declare `const priorSaveDeducted = loadedTreatmentStatus !== 'doctor-recorded' && loadedTreatmentStatus !== 'vitalsigns-recorded'` AND the reverse condition must include `&& priorSaveDeducted`. **Sanctioned**: none. **Cross-link**: `tests/v142-quater-doctor-finalize-over-credit.test.js` · TRUE-L2 `scripts/e2e-v142ter-doctor-finalize-course-deduct.mjs` (PHASE A typical / B buy / C over-credit / D V142-preserved).
