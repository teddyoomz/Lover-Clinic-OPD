...
## RG3 — Tier 2: Project docs + AGENTS.md + V-log

The project's own documentation and violation catalog may answer directly.
The V-log (`.Codex/rules/00-session-start.md`) may document a past
attempt at this exact problem.

Read, in order:
1. `AGENTS.md` — the master index; often links to the relevant doc
2. `.Codex/rules/00-session-start.md` — V-log for past pitfalls
3. `.Codex/rules/03-stack.md` (or equivalent) — stack-specific gotchas
...
1. **Deferred-tool list** (system-reminder) — search with `ToolSearch
   query:"<keyword>"`. Codex may already have the capability
...
3. **Anthropic skills bundle** — skills prefixed `anthropic-skills:*`
   ...
   setup-cowork) ship with Codex; check the available-skills list.
4. **Community skills** — search `site:github.com Codex skill <keyword>`
   via WebSearch.
5. **User-level skills** — if the same gap recurs across projects, this
   ...
   `~/.Codex/skills/` so every project inherits.
...
Register in `AGENTS.md` onboarding checklist: "when unsure, invoke
