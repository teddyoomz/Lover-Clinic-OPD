...
description: Find + install a public skill, MCP server, or tool to fill a capability gap — autonomously when the gap is safe, with user gate for anything paid / external-write / unfamiliar source. Covers 4 install paths — Anthropic bundled skills (already loaded), deferred tools (ToolSearch), MCP registry (mcp__mcp-registry), and public skill repos (git clone into ~/.Codex/skills/). Invoke when /research-gap RG6 indicates "capability missing, install required".
---
...
| Class | Example |
|---|---|
| **Built-in Codex tool** | file read, bash, grep, glob |
| **Deferred Codex tool** | WebFetch, NotebookEdit, computer-use, Chrome MCP |
...
## SA3 — Path 2: Deferred Codex tools (one-step load)
...
WebSearch query:"site:github.com Codex skill <keyword> SKILL.md"
...
# If approved:
cp -r /tmp/skill-candidate/<name> ~/.Codex/skills/<name>
```
...
**Never ask for:**
- Loading deferred Codex tools (Rule G)
...
2. **Grep `.Codex/settings.json`** (or equivalent) for the new
   registration
...
[SA1] Classify
    ...
    │
    ├── Deferred Codex tool?  Rule G auto-allows ──► SA3 ToolSearch
    │
...
→ Return: "User consent required. Install plan:
   Source: Official Slack MCP (github.com/slack/Codex-mcp, 1200★)
   Install: configure in ~/.Codex/settings.json with OAuth scope: chat:write
...
→ SA5 match: found github.com/someone/openapi-Codex-skill (30★)
...
→ Return: "User consent required, FLAG: recent repo, runs node script. 
  ...
  ~/.Codex/skills/ if OK. Or: use /skill-creator to build safer 
  in-project version."
```
...
→ Return: "Build in-project. Use Skill(skill:'anthropic-skills:skill-creator')
  or copy .Codex/skills/_template/SKILL.md → fill in invariants 
