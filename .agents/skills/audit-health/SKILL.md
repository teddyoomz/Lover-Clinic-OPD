...
Store output in `.Codex/audit-results/health-YYYY-MM-DD.md` for trend
tracking.
...
```bash
grep -cE "^### V[0-9]+ " .Codex/rules/00-session-start.md
```
...
```bash
ls .Codex/skills/audit-*/SKILL.md 2>/dev/null | wc -l
```
...
```bash
total=0
for s in .Codex/skills/audit-*/SKILL.md; do
...
**Store in `.Codex/audit-results/health-YYYY-MM-DD.md`** so next run
...
```bash
cat .Codex/settings.json 2>/dev/null | jq '.hooks | to_entries | map(.value | length) | add // 0'
```
...
grep -E "^### V[0-9]+ — " .Codex/rules/00-session-start.md | \
...
six_months_ago=$(date -d "6 months ago" +%Y-%m-%d 2>/dev/null || date -v-6m +%Y-%m-%d)
count=0
for s in .Codex/skills/*/SKILL.md; do
...
Write results to `.Codex/audit-results/health-YYYY-MM-DD.md`. Include
