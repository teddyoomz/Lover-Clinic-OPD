...
Read `.Codex/rules/00-session-start.md`. Find section 2 "PAST VIOLATIONS".
Find the highest V-number currently used. New entry = `V{N+1}`.

If section 2 doesn't exist (fresh Codex-guardrails install), create it
...
If solo: `git add .Codex/rules/00-session-start.md && git commit -m
...
A well-logged V-entry, read by a future team member or Codex session,
conveys:
