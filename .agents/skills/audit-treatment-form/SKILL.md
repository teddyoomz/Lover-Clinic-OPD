...
**Why**: scrollToError needs `data-field` on every validated field (AGENTS.md bug #8).
...
**Why**: AGENTS.md bug #2 — form dedups 156→50; index unsafe.
...
**Why**: AGENTS.md rule 2.
**Grep**: `{\\s*\\(\\s*\\(\\s*\\)\\s*=>` in TreatmentFormPage.
**Expected**: zero matches.

### TF9 — Stale closure on async-loaded props
**Why**: AGENTS.md rule 6 — clinic settings load async; effects depending on them need `loaded` flag or ref.
