...
1. `AGENTS.md` (project root) — stack, env, rule index, onboarding checklist
...
4. `.Codex/rules/00-session-start.md` — character + iron-clad rules A-H +
...
- If the next action touches a specific area (backend, UI, forms, ...),
  read the matching `.Codex/rules/0{N}-*.md` specialty file.
- If a memory system exists (e.g. `~/.Codex/projects/*/memory/MEMORY.md`),
  note it — Codex auto-loads; no manual Read needed.
...
- **Hooks**: if `.Codex/settings.json` has a SessionStart hook, it may
...
- **Memory**: `~/.Codex/projects/<proj>/memory/MEMORY.md` is auto-loaded by
  Codex. This skill does not re-read memory — Codex handles it.
- **Audit skills**: do NOT run `/audit-all` during session-start. That's
  a pre-release action, not a boot action.

## Success criteria

A fresh Codex session, after running `/session-start`, can:
