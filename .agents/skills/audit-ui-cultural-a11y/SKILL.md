...
### UC2 — Gold/yellow: SANCTIONED for VIP name/badge (superseded 2026-07-04)
**History**: the original "user dislikes gold" ban was REVERSED by user directive 2026-07-04
("ชื่อสีทองได้นะห้ามแดงเฉยๆ") — gold on names/badges is allowed; ONLY red stays banned (UC1).
**Now audit**: gold usage routes through `VipName`/`VipBadge` (`src/components/VipBadge.jsx`,
dark `#fbbf24` / light `#b45309` — AA both themes, V125 aaAccent pattern) or an equally
AA-checked amber pair (e.g. CustomerListTab VIP-sort chip amber-400 dark / amber-700 light).
Raw sub-AA yellow on a LIGHT background (e.g. bare `text-yellow-400` on white) is the violation.
