...
as source files. This skill greps the rule files (`.Codex/rules/*.md`)
...
```bash
grep -nE "^### [A-Z]\.|^### [A-Z]\.[0-9]" .Codex/rules/01-iron-clad.md
...
grep -nE "^### [A-Z]" .Codex/rules/01-iron-clad.md | while read line; do
  # Check next 20 lines contain "Why:" or "why:" or motivation
  file=".Codex/rules/01-iron-clad.md"
...
```bash
grep -rhoE "/audit-[a-z-]+" .Codex/rules/ | sort -u | while read skill; do
  name="${skill#/}"
  [ -f ".Codex/skills/$name/SKILL.md" ] || echo "[LR8] rule cites $skill — skill file missing"
done
```
...
```bash
refs=$(grep -rhoE "V[0-9]+" .Codex/rules/ | sort -u)
for v in $refs; do
  grep -q "^### $v " .Codex/rules/00-session-start.md \
...
for term in deploy commit push test build migrate; do
  count_always=$(grep -rcE "always.* $term|must.* $term" .Codex/rules/ | grep -v ':0$' | wc -l)
  count_never=$(grep -rcE "never.* $term|don't.* $term" .Codex/rules/ | grep -v ':0$' | wc -l)
