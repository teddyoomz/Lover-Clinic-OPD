...
import { SectionHeader, FormSection, ActionBtn, VitalsGrid, OPDFieldWithPrev } from './treatment-form/TfpFormPrimitives.jsx';
// TFP extraction step 2 (2026-07-07) — lab/med/med-group/remed/cons/cons-group
// modal JSX moved verbatim to ./treatment-form/TfpItemModals.jsx (state +
// handlers stay here, threaded as props; mount-conditionals stay at callsites).
// LabPriceSummary/MedPriceSummary now consumed there, not here.
    ...
    <div className={`fixed inset-0 z-[80] overflow-y-auto overscroll-contain ${isDark ? 'bg-[#0a0a0a] text-gray-200' : 'bg-gray-50 text-gray-800'}`}>
      ...
      <div className={selectedHistoryTreatmentId
        ...
        <div className="space-y-4 mt-4">
          ...
          {labModalOpen && (
            <LabItemModal
              {...{ isDark, labelCls, inputCls, selectCls, editingLabIndex, labModalLoading, labProducts,
                labModalSelected, setLabModalSelected, labModalQty, setLabModalQty, labModalPrice, setLabModalPrice,
                labModalDiscount, setLabModalDiscount, labModalDiscountType, setLabModalDiscountType,
                labModalVat, setLabModalVat, labItems, setLabItems, setLabModalOpen }}
            />
          )}
          ...
          <FormSection isDark={isDark}>
            ...
            {medModalOpen && (
              <MedItemModal
                {...{ isDark, labelCls, inputCls, editingMedIndex, medModalLoading, medFilteredProducts, selectMedProduct,
                  medModalQuery, setMedModalQuery, medModalSelected, setMedModalSelected, medModalQty, setMedModalQty,
                  medModalPrice, setMedModalPrice, medModalPremium, setMedModalPremium, medModalDiscount, setMedModalDiscount,
                  medModalDiscountType, setMedModalDiscountType, medModalVat, setMedModalVat,
                  medModalLabelOpen, setMedModalLabelOpen, confirmMedModal, setMedModalOpen }}
              />
            )}
            ...
            {medGroupModalOpen && (
              <MedGroupModal
                {...{ isDark, selectCls, medGroupSelectedId, setMedGroupSelectedId, medGroupData, medGroupLoading,
                  medGroupChecked, setMedGroupChecked, toggleMedGroupCheck, selectedGroupProducts,
                  confirmMedGroup, setMedGroupModalOpen }}
              />
            )}

            {/* Remed modal — past medications from treatment history */}
            {remedModalOpen && (
              <RemedPanel {...{ isDark, options, setMedications, setRemedModalOpen }} />
            )}
          ...
          <FormSection isDark={isDark}>
            ...
            {consModalOpen && (
              <ConsItemModal
                {...{ isDark, labelCls, inputCls, consModalLoading, consFilteredProducts, consModalQuery, setConsModalQuery,
                  consModalSelected, setConsModalSelected, consModalQty, setConsModalQty, confirmConsModal, setConsModalOpen }}
              />
            )}
            ...
            {consGroupModalOpen && (
              <ConsGroupModal
                {...{ isDark, selectCls, consGroupSelectedId, setConsGroupSelectedId, consGroupData, consGroupLoading,
                  consGroupChecked, setConsGroupChecked, toggleConsGroupCheck, selectedConsGroupProducts,
                  confirmConsGroup, setConsGroupModalOpen }}
              />
            )}
