// ─── TransferDetailModal — read-only detail view for a single transfer ──────
// Click row in StockTransferPanel → this modal shows full items, batch info,
// source+destination names, status history, notes.
...
export default function TransferDetailModal({ transferId, onClose }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [t, locs] = await Promise.all([getStockTransfer(transferId), listStockLocations()]);
      if (!t) throw new Error('Transfer not found');
      setData(t);
      setLocations(locs);

      // Fetch batch info for all source + destination batches
      const ids = new Set();
    ...
    <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-[var(--bg-surface)] rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="sticky top-0 z-10 bg-[var(--bg-surface)] border-b border-[var(--bd)] px-5 py-3 flex items-center gap-3">
          ...
          <h2 className="text-base font-bold text-[var(--tx-heading)]">รายละเอียดการโอนย้าย</h2>
          <span className="font-mono text-sky-400 text-sm">{transferId}</span>
          {data && (
          ...
          <div className="p-5 space-y-4">
            ...
            <div className="bg-[var(--bg-hover)]/50 rounded-xl p-4 grid grid-cols-2 md:grid-cols-3 gap-3 text-xs">
              <div>
                ...
                <div className="text-sm text-[var(--tx-primary)]">{locationName(data.sourceLocationId)}</div>
              </div>
              ...
              <div>
                ...
                <div className="text-sm text-[var(--tx-primary)]">{locationName(data.destinationLocationId)}</div>
              </div>
              <div>
              ...
              {data.deliveredTrackingNumber && (
                <div>
                  <div className="text-[10px] uppercase tracking-wider text-[var(--tx-muted)] font-bold mb-1">เลขพัสดุ</div>
                  <div className="font-mono text-sky-400">{data.deliveredTrackingNumber}</div>
                </div>
              )}
              {data.deliveredNote && (
                <div className="col-span-2">
                  <div className="text-[10px] uppercase tracking-wider text-[var(--tx-muted)] font-bold mb-1">บันทึกการส่ง</div>
                  <div>{data.deliveredNote}</div>
                </div>
              )}
              {data.canceledNote && (
              ...
              {data.rejectedNote && (
                <div className="col-span-2 md:col-span-3 bg-red-950/30 rounded-lg p-2 border border-red-900/50">
                  <div className="text-[10px] uppercase tracking-wider text-red-400 font-bold mb-1">เหตุผลการปฏิเสธ</div>
                  <div className="text-red-400">{data.rejectedNote}</div>
                </div>
              )}
            </div>
            ...
            <div>
              ...
              <div className="mt-2 text-[10px] text-[var(--tx-muted)] flex items-start gap-1">
                <AlertCircle size={11} className="flex-shrink-0 mt-0.5" />
                <span>เมื่อรับของ (status 1→2) ระบบจะสร้าง <b>batch ใหม่</b> ที่ปลายทาง โดยสืบทอดต้นทุน+วันหมดอายุจาก batch ต้นทาง</span>
              </div>
            </div>
          </div>
        )}
