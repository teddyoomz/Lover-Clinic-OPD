...
// Firestore-only CRUD (per CLAUDE.md rule E: Backend ใช้ข้อมูลจาก Firestore
...
//
// Modal chrome extracted to MarketingFormShell + id/scrollToField to
// marketingUiUtils (AV10).
...
export default function PromotionFormModal({ promotion, onClose, onSaved, clinicSettings, isDark }) {
  ...
  // ProClinic-synced items (master_data/{type}/items/*).
  ...
  const handleSave = async () => {
    ...
    try {
      const id = isEdit ? (promotion.promotionId || promotion.id) : generateMarketingId('PROMO');
  ...
  return (
    <MarketingFormShell
      isEdit={isEdit}
      titleCreate="สร้างโปรโมชันใหม่"
      titleEdit="แก้ไขโปรโมชัน"
      onClose={onClose}
      onSave={handleSave}
      saving={saving}
      error={error}
