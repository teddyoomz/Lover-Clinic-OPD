# LoverClinic App — Codex Master Index
...
**⭐ MANDATORY FIRST READ**: [`.Codex/rules/00-session-start.md`](.Codex/rules/00-session-start.md) — character, expectations, all iron-clad rules, past violations, tool/skill decision tree, workflow checklist. Read every new session + after any compaction.

| ไฟล์ | ใช้เมื่อ |
|---|---|
| [`00-session-start.md`](.Codex/rules/00-session-start.md) | **ทุก session start** — single-source summary of everything |
| [`01-iron-clad.md`](.Codex/rules/01-iron-clad.md) | ทุก turn — A/B/C/D (+ E backend-firestore-only referenced from 00) |
| [`02-workflow.md`](.Codex/rules/02-workflow.md) | Commit / push / deploy / test |
| [`03-stack.md`](.Codex/rules/03-stack.md) | Firestore / Vite / React / Backend / ProClinic / Chat gotchas |
| [`04-thai-ui.md`](.Codex/rules/04-thai-ui.md) | UI / colors / dates / Thai culture |
...
- **J. 🆕 Superpowers Auto-Trigger** (2026-04-29 + extended session 33) — ทุก session start: invoke `using-superpowers` skill ก่อน (boot). Mandatory skill invocation BEFORE any other tool call เมื่อ context ตรง:
  - งานสร้าง feature / component / endpoint / API ใหม่ → `brainstorming` (HARD-GATE — ห้ามเขียนโค้ดก่อน design approval). **🆕 Plan-mode = ORTHOGONAL** (locked 2026-04-29 session 33 หลัง user catch drift): เข้า plan mode **ไม่** auto-suppress brainstorming. ต้องเรียก `Skill(brainstorming)` ก่อนเขียน plan file เสมอ — แม้ system-reminder จะบอกว่า plan mode active. Plan-mode workflow (Explore → AskUserQuestion → plan file → ExitPlanMode) **ไม่ใช่** brainstorming-equivalent — เป็นคนละ layer (IDE permission sandbox vs methodology Q&A). ดู `.Codex/rules/00-session-start.md` Rule J expanded.
  ...
  - User explicit instruction (AGENTS.md / iron-clad A-I) override skill behavior เมื่อ conflict (per `using-superpowers` Instruction Priority).
...
## 📋 Onboarding สำหรับ Codex แชทใหม่
...
4. **`.Codex/rules/00-session-start.md`** ⭐ — character + all iron-clad rules A-H + H-bis + past violations (V1-V9)
5. **`.Codex/rules/01-04`** — detail rules (auto-load ตาม context)
...
> Memory: `~/.Codex/projects/F--LoverClinic-app/memory/` (ดู `MEMORY.md` index) — auto-loaded ทุก session
> `.agents/` layer: installed 2026-04-20 via agent-context-kit. Scaffold policy ใน `.agents/AGENTS.md`. Iron-clad rules ใน `.Codex/rules/` ชนะเสมอเมื่อ conflict.
