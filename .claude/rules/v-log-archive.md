...
#### §followup — Post-ship CRITICAL: canvas init re-run destroyed the React-owned `<canvas>` (2026-05-21 EOD+1 LATE)

The user's on-device L1 (the piece I'd flagged as harness-limited) hit **3 symptoms** the moment they used the editor: (1) the PC-selected template doesn't show (blank white canvas), (2) no tool draws anything, (3) save does nothing. `/systematic-debugging`, Iron Law honored — root cause found in a REAL browser BEFORE any fix, and **two hypotheses were REJECTED by real-browser evidence** (which is exactly why the Iron Law exists):

- **Rejected H1 (re-init mechanism is fragile)**: an isolated eval of `new fabric.Canvas` → `dispose()` (un-awaited) → 30ms → `new fabric.Canvas` on the SAME element worked cleanly (1 wrapper, template loads, export works). So raw Fabric re-init is fine.
- **Rejected H2 (SVG template 0-natural-size → scale Infinity)**: the real chart SVGs carry explicit dimensions (face 800×1000, body 600×1200); `FabricImage` + add + render + export all worked. So the template-load primitive is fine.

**Confirmed root cause** — a temp probe (`tabletprobe.html` + `src/__tabletProbe.jsx`) that MOUNTED THE REAL `TabletChartCanvas` and drove the late-template race (`setTpl('')` → `setTpl(dataURL)`) reproduced it deterministically: after the late template, `wrappers:0, lower:0, exportFabricJson:null, exportDataUrl:null` — **the canvas was GONE and `fcRef` was null**. The init `useEffect` was keyed on `[templateImageUrl]`; the template arrives LATE via the relay's instant-pop race (the very race the prior bugfix saga added the late-template handler for), so the effect RE-RAN → its cleanup `fc.dispose()` removed the `<canvas>` element React owns (Fabric v7 takes over + removes that DOM node; dispose is async and isn't awaitable in a sync cleanup) → the re-init's `elRef.current` was null → `fcRef` never re-set → blank template + no handlers (no draw) + `exportDataUrl()` returns null (no save). **One root cause → all three symptoms.** The proven PC `ChartCanvas` never hits this because it inits ONCE (`[]` deps); my isolated L1 (raw fabric), L2 (transport), and even my prior "L1 real-browser" (primitives only) all passed because NONE of them exercised the **mounted-component prop-change lifecycle**.

**Fix**: init the Fabric canvas **exactly once** (`useEffect(..., [])`, eslint-disabled, mirror PC `ChartCanvas` + old `PenCanvas`) + a separate `useEffect(..., [templateImageUrl])` calls `loadTemplate(url)` which adds/replaces the locked template image on the **LIVE** `fcRef` canvas (and `fc.setDimensions` to fit the ratio) — it NEVER disposes or `new fabric.Canvas`. The init effect itself calls `loadTemplate(templateUrlRef.current)` at the end so the late-arriving value (read via an always-current ref) is captured race-free. History is re-baselined inside `loadTemplate` so undo can't remove the template. **Probe re-verify post-fix**: after the late template, `wrappers:1, lower:1, json:['Image']` (template loaded) and the canvas survives an idempotent re-set + a tool change. RC1-RC3 source-grep regression lock the structure (init-`[]`-deps + separate `[templateImageUrl]` effect + `loadTemplate` has no `dispose`/`new fabric.Canvas`). Full vitest 13924 → **13927/0**; build clean. Temp probe + harness files deleted (never committed). NOT deployed (await "deploy").

**Lessons (V66 reaffirmed, component-lifecycle layer)**:
- **"Engine verified in isolation" ≠ "mounted component works."** The Fabric engine, the transport, and the per-tool object creation were ALL genuinely verified — and ALL irrelevant to this bug, which lived purely in the React↔Fabric **lifecycle** (effect deps → dispose → DOM-ownership). The honest-scope note in the original entry ("event-wiring is harness-limited → user on-device L1") was correct that something was unverified, but UNDER-estimated the risk: the unverified piece hid a total-failure bug, not a polish gap.
- **A library that takes over a React-owned DOM node (Fabric, a chart lib, a video player) MUST be initialized ONCE.** Reactive prop changes mutate the live instance in separate effects; putting the prop in the init effect's deps causes dispose+re-init that fights React for the element. This is a reusable class-of-bug (DOM-library-init-deps).
- **When a component bug can't be driven by the harness, build a temp probe that MOUNTS the real component and drives the real prop lifecycle.** Raw-library evals + transport e2e are necessary but mount-blind. The probe (mount real component → drive `templateImageUrl` `''`→dataURL → inspect `fcRef`/DOM via the exposed ref) is what caught it — and is the pattern for future "the primitives work but the mounted thing is broken" bugs.
- **The Iron Law paid for itself twice this session**: two confident hypotheses (re-init, SVG-size) were both wrong and would have shipped non-fixes; only the real-browser repro found the truth.

#### §followup-2 — Post-ship SAVE bug: storage.rules denied result.json; only a full-relay e2e caught it (2026-05-21 EOD+1 LATE+1)

After §followup shipped the init-fix, the user re-tested and reported "ยังไม่หาย อาการเหมือนเดิมเป๊ะ" + the decisive directive: **"เวลามึงทดสอบ มึงต้องให้มันปรากฎรูปที่ได้จาก PC จริง → ทดลองใช้เครื่องมือวาดจริง → ส่งกลับไป PC ได้จริงพร้อมทุกสิ่งที่แก้ไข … เทสเหมือนคนใช้จริง e2e ด้วย."** They were 100% right: I had "verified" the init-fix by ref-inspection (`exportFabricJson()` returns json with `Image`; `exportDataUrl()` returns a PNG string) — never a real save→PC round-trip, never a trusted-event draw, never a visible-render check.

**The right tool = Playwright** (it dispatches **trusted** pointer events Fabric processes; `preview_eval` synthetic events are `isTrusted:false` and Fabric ignores them — which is why I couldn't drive drawing earlier and leaned on ref-inspection). The e2e helper (`tests/e2e/helpers.js`) already signs in as `loverclinic@loverclinic.com` + injects the token, so the **real authenticated `?tablet=chart` route** is drivable. Built `tests/e2e/tablet-chart-more-tools-relay.spec.js`: admin-SDK PC creates the session + uploads the real `face-male.svg`; an authed Playwright tablet (known injected `chartTablet:deviceId`; branchId read back from the presence doc) instant-pops → draws with `page.mouse` (trusted) → saves → the PC side reads the result via admin SDK. Pixel-level + screenshot proof.

**Run 1 evidence**: STEP3 template **205 content pixels** (renders), STEP4 pen **52 red pixels** (draws) — so the init-fix genuinely fixed template+draw. STEP5 **FAILED**: after clicking save, the PC never got `status=saved`/`resultImageUrl`. So **save was a SEPARATE, still-live root cause** (the init-fix had merely un-masked it — pre-init-fix the dead canvas made `exportDataUrl()` return null, so the json upload was never even attempted; once the canvas was alive, the json upload ran and was denied).

**Root cause**: `storage.rules` generic `match /uploads/{collection}/{docId}/{fileName}` allows `create` ONLY for `image/*` or `application/pdf`. The NEW `result.json` (`application/json`), uploaded by the tablet via the **client SDK** (`uploadTransportJson` → `uploadString`), was **DENIED** → it threw → `onSave`'s `Promise.all([uploadImage, uploadJson])` rejected → `updateChartEditSession({status:'saved'})` never ran → the session stayed `active` → silent save failure. **The admin-SDK L2 e2e (T7, `e2e-tablet-chart-more-tools.mjs`) PASSED because admin SDK `bucket.file().save()` BYPASSES storage rules** — the client path is subject to them. This is the **V66 admin-vs-client blind spot a 3rd time** (V66 itself: admin doc-level bypasses composite indexes; V81-fix1: admin Timestamp shape; here: admin bypasses storage rules).

**Fix (2-part)**: (1) `storage.rules` — a specific `match /uploads/chart-edit-sessions/{sessionId}/{file=**}` allowing `image/*` + `application/json` (staff-only, 10MB; Storage OR-grants across matching rules). Needs `firebase deploy --only storage` (Probe-Deploy-Probe #13). (2) `onSave` — the lossless fabricJson upload is **non-fatal** (`try { jsonUrl = await uploadTransportJson(...) } catch { jsonUrl = null }`) so a json failure NEVER blocks the PNG save (the flattened PNG already carries every visible edit into `charts[]`); plus a top-level try/catch surfacing a visible save-error (`saveErr` in the header) — no more silent failure. **Run 2 (post fix-2, rules NOT yet deployed)**: STEP5 **PASS — `resultImageUrl` set, 123KB PNG (template + pen + rect) → PC**; STEP6 logs "json deferred to storage.rules deploy" (graceful). RC4 (storage.rules allows json) + RC5 (onSave json non-fatal + visible error) lock it. Full vitest 13927 → **13929/0**; build clean.

**Lessons**:
- **Ref-inspection is not verification.** "The function returned a string / the object has an Image" says nothing about whether the real save reaches the PC. Only a real user-flow e2e (real PC image → trusted draw → real save→PC) proves a relay feature. The user's demand was exactly the V66 standard, applied to me again.
- **For ANY upload/relay feature, the e2e MUST use the CLIENT SDK or a real authed browser — never admin SDK alone.** Admin SDK bypasses storage + firestore rules, so an admin-SDK "e2e" cannot see the rule denial that breaks the real client. (V66 / V81-fix1 / this = the same blind spot three times — escalate: any feature whose client writes to a rules-gated path needs a client-side e2e gate.)
- **A multi-upload save must make the non-essential parts non-fatal.** `Promise.all` couples the PNG (essential) to the json (optional) — one failure killed both. Guard the optional upload; never let it block the essential write.
- **The full-relay Playwright pattern is now the canonical e2e for the tablet chart relay**: admin-SDK PC side + authed Playwright tablet + trusted `page.mouse` draw + pixel/screenshot assertions + PC-side result verification. Reusable for any future relay change.

#### §followup-3 — Post-ship LIVE-DISPLAY bug: the tablet canvas never painted on screen (rAF-deferred render); verified at the rendered-pixel level + forced dpr=2 (2026-05-21 EOD+1 LATE+2)

After §followup-2 made save work, the user re-tested and reported a NEW, distinct symptom (with two screenshots): the **save is now correct** (the PC chart shows the template + every pen/tool edit), **but the iPad editor screen renders NOTHING live** — "กดอะไรวาดไปก็ขาวเหมือนเดิม แม้แต่รูปที่เลือกมาจาก PC ก็ไม่ปรากฎ … แปลว่ามันรับ input แล้ว แต่ไม่แสดงผลสดๆขณะทำการแก้ไขในจอ tablet". So: input is received + the object model is correct (save proves it), but the visible canvas is blank (template + every stroke invisible) during editing.

`/systematic-debugging`, Iron Law honored. **Phase 1 — rendered-pixel evidence (the layer my §followup probe skipped).** My §followup probe had checked `json:['Image']` (object model present) and "passed" — but that's the V66 trap a 4th time: object-model-present ≠ pixels-painted. This time I read ACTUAL pixels. Built a probe that mounts the REAL `TabletChartCanvas`, **forces `config.devicePixelRatio=2`** (Fabric 7.2.0 `config` is settable → reproduces the iPad retina path on a dpr=1 desktop), simulates the late-template relay race, and reads the visible lower-canvas via `getImageData` (12×12 grid: white/transparent/colored counts).
- First measurement was contaminated by a **5px-wide preview viewport** (a probe artifact — the skill's "is that the environment, not the bug?" check); resized to 1194×834 (iPad landscape) and re-measured.
- Real reproduction at dpr=2: lower-canvas correctly sized **389×778 CSS** (backing 778×1556, ratio 0.5 — exactly the user's full-size white rectangle), object model `["Image"]`, but **0 painted px (all 121 samples transparent)**. The `<canvas>` has CSS `background:#fff`, so a transparent canvas reads as white on screen — the exact symptom.
- **Contrast at dpr=1**: ALSO transparent — so the bug is NOT retina-specific (the on-screen render had been broken at every dpr; my dpr=1 §followup probe only "passed" because it checked the model).

**Phase 1 isolation** (rule out probe artifacts): a STANDALONE `fabric.Canvas` (independent of the component) painted both a Rect AND the same SVG-data-URL image at dpr=2 (`afterRect` 25 colored; `afterImg` 121 colored) → Fabric, retina, the SVG image decode, and `getImageData` readback ALL work in this browser. So the bug is in the COMPONENT, not Fabric/image/browser. Then, via a non-invasive prototype hook capturing the component's live `fc`: `fc.lowerCanvasEl === document` lower-canvas (single instance, in DOM, correct backing) — and calling **`fc.renderAll()` (SYNC) painted the template** (transparent→colored 121), while the component's own path left it blank. A further test showed **`requestAnimationFrame` does NOT fire in this headless preview** — so the component's rAF-deferred renders never ran here.

**Phase 2 — compare to the proven-working reference.** `grep` of render calls: the PC **`ChartCanvas` (works) uses synchronous `canvas.renderAll()` ×6, ZERO `requestRenderAll`**; my **`TabletChartCanvas` (broken) used `fc.requestRenderAll()` ×17** (rAF-scheduled), with sync `renderAll` only once at init. **Root cause**: the component deferred every paint (template load, pen render, every shape/text/erase, undo/redo/clear) to a `requestAnimationFrame` tick via `requestRenderAll`. On the tablet that rAF is unreliable — throttled, a stuck Fabric `nextRenderHandle` (once set, subsequent `requestRenderAll` calls are no-ops until the rAF fires), or simply not firing in some editor states (and provably DEAD in the headless preview). So the deferred paint never landed → template + strokes invisible. `toDataURL` save renders the object model to a FRESH canvas (bypasses the on-screen render entirely) → save stayed correct, masking the bug across the whole saga.

**Phase 4 — fix + verify.** Replaced all `fc.requestRenderAll()` → `fc.renderAll()` (sync; mirror the proven `ChartCanvas`). Sync render is rAF-independent so it always reaches the screen. **Re-verified in the browser at dpr=2 with rAF still dead**: the component's own render path now paints the template (`colored:121`, was transparent) AND a stroke added through `commitPen`'s exact `fc.add + renderAll` paints live (sampled gray `[160,160,160]` → green `[0,238,0]`). That a paint lands even with rAF completely dead is the strongest possible guarantee for the iPad (whatever its rAF state). **RC6-RC8** lock it (0 `requestRenderAll` token in `TabletChartCanvas`, ≥15 sync `fc.renderAll(`, the `ChartCanvas` reference contract, and the rationale comment). **AV104** invariant: Fabric canvas editor components MUST paint via synchronous `renderAll`, never the rAF-deferred path. Rule P class-of-bug grep: only `TabletChartCanvas` used `requestRenderAll` (1 instance, no siblings; `ChartCanvas` already sync, `PenCanvas` used `isDrawingMode`/PencilBrush which Fabric renders internally). Full vitest **13932/0** (+3 for RC6-RC8); build clean. Temp probe files deleted (never committed). NOT deployed (await "deploy").

**Lessons**:
- **Verify RENDERED PIXELS, not the object model — V66, 4th time in this saga.** My §followup probe checked `exportFabricJson()` → `['Image']` and declared the canvas fine; the screen was blank the entire time. "The object is in the model" / "the function returned data" is NOT "the user sees it". Read `getImageData` on the live canvas.
- **Reproduce device-only render bugs by forcing the device condition.** `config.devicePixelRatio=2` reproduced the iPad retina path on desktop; that the bug ALSO showed at dpr=1 (once I read pixels) told me it wasn't retina at all — a cheaper, truer localization than guessing.
- **Watch for environment-vs-bug.** The 5px-wide preview viewport nearly looked like a "collapsed canvas" root cause; resizing to a real iPad viewport showed the canvas was correctly sized and the real bug was zero-paint. Always sanity-check the harness (viewport, rAF, dpr) before trusting a measurement.
- **A library that takes over the canvas must paint synchronously when correctness depends on it.** `requestRenderAll` is a perf optimization (coalesce many renders into one rAF) that silently fails when rAF doesn't fire. The proven reference (`ChartCanvas`) renders synchronously; mirror the reference, don't diverge to a "more efficient" path that has a hidden liveness dependency.
- **Phase 2 "compare to the working example" was the decisive step** — the same feature exists and works on the PC; the one structural difference (sync vs rAF render) WAS the bug. When a sibling works and yours doesn't, diff them before theorizing.

#### §followup-4 — Post-ship LIVE-DISPLAY bug ROUND 2: the REAL on-device cause was an opaque upper-canvas cover (Fabric copies the element's inline `background`) (2026-05-21 EOD+1 LATE+3)

After §followup-3 (sync-render) I claimed the live-render bug fixed (verified in the headless preview). The user re-tested **on the device** and replied, terse + frustrated: **"เหมือนเดิมเป๊ะ ยังไม่หาย"** (exactly the same, still broken). Fourth "verified" claim wrong on the real device → systematic-debugging architectural-questioning territory; STOP guessing, get REAL evidence.

**The confound that broke fix3.** A clarifying question (the user authorized working without stopping, but a wrong assumption had silently undermined every fix) established the user tests on **localhost dev from a real browser — where `requestAnimationFrame` IS alive** (the DIAG later showed `raf/s 145`). My §followup-3 reproduction was in the **headless Claude Preview where rAF is DEAD** — a different condition. So `requestRenderAll` WOULD have painted on the user's real browser; sync-render fixed a **preview-only artifact**, not the device bug. Lesson burned in: a headless preview's dead-rAF is a confound — verify in a browser with rAF alive (Rule S, added this session: Chrome MCP / real-browser standing authorization).

**Getting on-device evidence I otherwise can't.** I can't open devtools on the user's device, so I added a TEMP on-screen DIAG overlay to the editor that reads the REAL visible lower-canvas (`getImageData`) + dims + dpr + object count + rAF liveness. The user screenshotted: `DIAG dpr 1.4 · back 1446x1807 · css 1033x1291 · objs 7 · paint c7 w42 t0/49 · canvases 2 · raf/s 145`. Read: the canvas is correctly sized (css 1033×1291), the object model has 7 objects (template + 6 strokes), rAF is alive (145/s), and crucially **the lower-canvas IS painted — `t0` (0 transparent), not the pre-fix `t49`** (so sync-render DID make it paint) — yet the screen is white. Painted backing + correct save + white screen ⇒ **something opaque is covering the painted lower-canvas.**

**Root cause (proven in a real browser via a fabric-reexport isolation test).** Recalling that my earlier `__analyze` had measured BOTH the lower-canvas AND the upper-canvas with computed `background: rgb(255,255,255)`, I tested Fabric's upper-canvas creation directly: make a `fabric.Canvas` on an element WITH inline `background:#fff` vs WITHOUT, then read the upper-canvas. Result: **WITH inline bg → the upper-canvas inline+computed background is white** (Fabric v7 copies the lower canvas element's inline `style` — including `background` — to the upper-canvas); **WITHOUT → the upper-canvas is transparent (`rgba(0,0,0,0)`)**. The upper-canvas is the interaction layer, **absolutely positioned ON TOP** of the lower-canvas. So the component's inline `background:#fff` → an **opaque white upper-canvas that COVERS everything painted on the lower-canvas** → blank-white screen. The `toDataURL` save reads the lower-canvas object model → correct → masking the cover the whole time.

**Class-of-bug grep (Rule P).** `grep "<canvas" src/ | grep background` → `TabletChartCanvas` was the ONLY canvas element in `src/` with an inline background. The proven PC `ChartCanvas` renders `<canvas ref={canvasElRef} className="shadow-lg" />` — **no inline background at all** (it relies on Fabric `backgroundColor`). THAT is precisely why ChartCanvas works on the PC and the tablet didn't. 1 instance, no siblings.

**Fix.** Remove the inline `background:'#fff'` from the `<canvas>` element (mirror ChartCanvas). The white fill already comes from Fabric `backgroundColor:'#fff'` in the init, which paints the LOWER canvas backing (not a CSS background on the element that propagates to the opaque cover). RC9-RC11 lock: the element JSX has no `background`; the init has `backgroundColor:'#fff'`; the ChartCanvas reference has no inline canvas background. **AV105** invariant (companion to AV104). Isolation test confirms the fix: WITHOUT inline bg → upper-canvas transparent → the lower-canvas content shows through. Temp DIAG overlay + the bgprobe harness deleted (never committed). Full vitest GREEN; build clean. NOT deployed (await "deploy").

**On fix3 (sync-render).** KEPT, not reverted — it mirrors the proven `ChartCanvas` (which renders synchronously) and is rAF-independent defensive hardening (robust if a real device ever throttles rAF, e.g. a backgrounded tab). Honest record: it addressed a preview-only artifact, not this bug; the actual on-device cause was the cover (this entry). Both fixes are correct and both match the working reference.

**Lessons**:
- **Verify in a browser with rAF ALIVE.** The headless Claude Preview's dead-rAF was a confound that sent fix3 down the wrong path and produced a false "verified". Rule S now mandates real-browser verification; the on-screen DIAG overlay is the pattern for devices whose devtools I can't open.
- **Fabric copies the canvas element's inline style to the opaque upper-canvas.** NEVER put an opaque `background` (or any cover-causing style) on a Fabric-wrapped canvas element — set the fill via Fabric `backgroundColor` (paints the lower canvas). AV105 locks this.
- **The working sibling had the answer all along.** ChartCanvas works and has no inline canvas background; one `grep "<canvas"` diff would have found this on round 1. "Compare to the working example" (Phase 2) is not optional — do it FIRST, structurally (diff the actual element/markup), not just conceptually.
- **"Painted backing + correct save + blank screen" ⇒ a cover, not a render failure.** When `getImageData` on the backing shows content but the screen is blank, stop chasing the render path and look for an opaque layer on top (sibling canvas, overlay, z-order). The DIAG's `t0` (fully opaque, nothing transparent) was the tell that the canvas WAS painting — the problem was above it.
- **Four rounds (init / save / sync-render / cover) on one feature = the architecture warning fired for real.** Each "fix" was verified at a layer that wasn't the user's actual condition (mock → admin-SDK → headless-preview). Only real-device evidence (the DIAG screenshot) + a real-browser isolation test localized it. The discipline that finally worked: real environment + read the actual pixels/DOM + diff the working sibling.

#### §followup-5 — Real-use round-trip verification → object-level RE-EDIT completed + 1MB-persist size guard (2026-05-21 EOD+1 LATE+4)

After the user confirmed fix4 ("โอเคใช้ได้แล้ว"), they asked for comprehensive verification of the real-use flows + bug/edge-case hunting (verbatim): *"Chart ที่ได้ไปจาก ipad สามารถบันทึกลง OPD คนไข้ได้จริง กลับมา Edit ทีหลังได้ และใช้กับรูปที่เพิ่งอัพเข้าไปใน pc แล้วส่งมา edit ได้ ลองทำเทส round trip, stimulate, e2e, stress เพื่อหาบั๊คและ edge case ที่เหลือให้เจอจนได้ และแก้ไขให้ทำงานได้แบบรองรับทุกสภาพปัญหา"*.

**Phase 1 (Explore map) — the lifecycle.** Tablet save → `useChartEditSession` SAVED handler downloads `resultImageUrl` + `resultFabricJsonUrl` → `onSaved({dataUrl, fabricJson, templateId, source})` → `ChartSection.handleSave` merges into `charts[]` (max 2) → on the clinician's "Save Treatment" click, `TreatmentFormPage` persists `detail.charts = charts.map(c => ({dataUrl, fabricJson, templateId}))` → `setDoc(treatmentDoc(BT-...))` in `be_treatments`. So charts persist into the patient's OPD record on treatment-save (not relay-save). Fresh PC image: `ChartTemplateSelector` reads the file → data URL → relay `uploadTransportImage('template')` → tablet downloads.

**Real-prod round-trip e2e — DATA LAYER SOLID (14/0).** `scripts/e2e-chart-relay-roundtrip.mjs` drives REAL Firestore + Storage (admin SDK for verify; client-fetch path for downloads): (A) fresh template upload → session → tablet result PNG+json → PC download → persist to `be_treatments.detail.charts[]` → re-read → every object type survived to the OPD record; (B) re-edit (parse persisted json → add a stroke → re-persist → re-read, +1 object); (C1) large json 68 objects intact + <1MB; (C2) special-char Textbox (2026 chars: emoji/Thai/RTL/zero-width/newline) byte-identical after the Storage+Firestore round-trip; (C3) two concurrent sessions persist to distinct patients, no cross-contamination; (C4) rapid re-save → last-write-wins, no corruption. Cleanup zero orphans. **Honest scope (Rule Q V66)**: admin SDK bypasses storage.rules, so this proves the DATA contracts + the PNG path (image/png is allowed for the client too); the CLIENT `result.json` upload is rules-gated until the storage deploy (verified separately by the real-browser relay spec).

**The real gap (the user's "กลับมา edit ทีหลังได้").** Reading `ChartCanvas.jsx:60` showed PC re-edit loaded the flattened PNG as a flat Fabric background and **never read `existingData.fabricJson` / never `loadFromJSON`** (comment: "no loadFromJSON hang"). So re-edit was **raster-only** — the prior strokes were baked into the image, not movable/erasable objects — defeating the entire AV103 lossless transport. The fabricJson was transported + persisted but **never consumed**.

**Fix (object-level re-edit).** The hard part is the coordinate space: fabric objects carry ABSOLUTE coords, so loading a tablet json into a differently-sized PC canvas misplaces everything. Solution: a shared `serializeFabricCanvas(fc)` (in `tabletChartTools.js`) embeds `canvasWidth`/`canvasHeight` in the json; BOTH `TabletChartCanvas.exportFabricJson` + `ChartCanvas.handleSave` use it. `ChartCanvas` re-edit now: `isObjectLevelReeditable(json)` (objects + dims present) → `initCanvas` at the SAVED dims → `await loadFromJSON` → re-lock object[0] (template) + force white bg → render; **graceful PNG-raster fallback** when there's no re-editable json (legacy charts, or pre-storage-deploy when the tablet json upload is denied → `fabricJson` null). No zoom (native dims, scroll if large) → object coords stay consistent across re-edits (zoom + re-save would corrupt the stored dims). **Verified in a REAL browser**: mounted the actual `ChartCanvas` with a tablet-style json → `objectLevelPathTaken:true` (canvas sized to the json's native 600×800, NOT fit-to-container) + objects rendered (`paint colored 27, white 142, transparent 0` after the white-bg fix). Rule Q satisfied at the pixel level.

**Edge case found + guarded (the 1MB Firestore doc cap).** The chart PNG dataUrl AND the fabricJson are BOTH inlined into the `be_treatments` doc (Firestore caps a doc at ~1MB). A fabricJson with a big EMBEDDED template (fabric `toJSON()` embeds the image `src` as a data URL) could push the doc over 1MB → the entire treatment save fails. NEW `chartEntryForPersist(c)` size guard: always keep the PNG dataUrl + templateId, but DROP the fabricJson when `dataUrl.length + fabricJson.length > 700KB` → the chart still saves + displays; that one chart re-edits as raster. TFP persist now `.map(chartEntryForPersist)`. **Known pre-existing limit (flagged, NOT introduced here)**: a single chart PNG dataUrl that ALONE exceeds ~1MB still risks the cap — inlining chart images in the Firestore doc predates the relay; the architectural follow-up is Storage-ref (store chart blobs in Storage, reference by URL in the doc). The guard prevents the NEW fabricJson from compounding it.

**Artifacts.** `scripts/e2e-chart-relay-roundtrip.mjs` (real-prod 14/0). `tests/chart-relay-roundtrip.test.js` (U1-U3 unit: serialize-with-dims / isObjectLevelReeditable / chartEntryForPersist; SG1-SG3 source-grep: ChartCanvas object-level consume + both serializers + guarded persist; F1 Rule I flow-simulate: tablet→transport→persist→re-read→re-edit→re-export incl. legacy + huge-template cases). AV103 follow-up locks the 3 boundaries. Full vitest GREEN; build clean. Temp probes (chartreeditprobe) deleted.

**Lessons**:
- **Transporting data is pointless if the consumer ignores it.** The fabricJson was faithfully transported + persisted + tested at the transport layer (AV103) for the whole feature life — while the re-edit consumer silently dropped it for the flattened PNG. Verify the CONSUMER end-to-end, not just the producer/transport. (Same family as the V66 "verify the real flow" lesson, at the consume boundary.)
- **Fabric object coords are absolute → re-edit must carry + recreate the canvas dimensions.** Loading objects into a differently-sized canvas misplaces them; embed the dims in the serialized json + recreate the same space (no zoom, or you corrupt the dims on re-save).
- **Inlining images/json in a Firestore doc has a hard ~1MB ceiling.** Guard the inline size (drop the optional part, keep the essential) so a large item can't break the whole save; the real fix for large media is Storage-ref, not inlining.
- **A clean real-prod round-trip e2e (admin for verify + client-fetch for download) is the right L2 for a persist+transport feature** — it proved the data layer solid in one run and let me focus the fix on the one real gap (the UI consumer).

**Status**: object-level re-edit is **live-gated on the storage.rules deploy** (until then the client `result.json` upload is denied → `fabricJson` null → raster fallback, which works). Post-deploy: tablet → save → PC re-edit shows prior strokes as movable objects. NOT deployed (await "deploy").

#### §followup-6 — Re-edit a saved chart ON THE TABLET (feature; object-level hydrate via the relay) (2026-05-21 EOD+2)

§followup-5 completed object-level re-edit on the PC (`ChartCanvas` consumes the persisted `fabricJson`). The user's next ask was the tablet side: clicking edit (✏️) on a saved chart should offer the same PC/tablet choice as add-new, and re-editing on the iPad should load the chart's PRIOR annotations + merge back into the same slot. Design was brainstormed + approved the prior session ("โอเค ลุยเลย"); this session implemented it: `brainstorming` HARD-GATE (already satisfied) → spec HTML → `writing-plans` HTML → `executing-plans` inline (6 tightly-coupled files; subagents thrash on this baseline per V81/V86). Spec/plan: `docs/superpowers/{specs,plans}/2026-05-21-re-edit-saved-chart-on-tablet*`.

**Architecture — thread ONE field through the proven relay (zero new infra).** Before this, edit (✏️) went straight to the PC `ChartCanvas`; new charts already showed `PcPairingModal`. The change makes edit consistent + adds the tablet leg:
- `chartEditSessionCore.buildSessionCreate` gains `editFabricJsonUrl: null` (the only schema delta).
- `useChartEditSession.start({editFabricJson})` — after uploading the chart PNG as the `template`, uploads the chart's `fabricJson` as `edit.json` via the EXISTING `uploadTransportJson('edit')` (GUARDED in try/catch → a json-upload failure NEVER blocks the relay; the tablet falls back to the PNG raster), then patches `{templateImageUrl, editFabricJsonUrl}` in ONE update (no intermediate flash).
- `TabletChartEditorPage` — NEW `initialFabricJson` state + a single `resolveSource(doc)` (used for both the initial open + the instant-pop late-arrival) that resolves the edit json FIRST: if `editFabricJsonUrl` downloads to an `isObjectLevelReeditable` json → `setInitialFabricJson` + RETURN (skip the raster PNG → the canvas never double-loads); else → the `templateImageUrl` PNG. Passes both props to the canvas.
- `TabletChartCanvas` — NEW `initialFabricJson` prop + `hydrateFromJson` (mirror PC `ChartCanvas:69-79`: `setDimensions(canvasWidth, canvasHeight)` so the re-edit recreates the SAME coordinate space — fabric objects carry ABSOLUTE coords — then `loadFromJSON` + force white bg + re-lock object[0] as the template + baseline history). The init-once effect picks json-vs-template at the end; the `[templateImageUrl]` effect EARLY-RETURNS when a reeditable json owns the canvas (the no-double-load gate); a new `[initialFabricJson]` effect hydrates on late arrival.
- `ChartSection` — NEW `pendingChart` state; `handleEdit(idx)` now stages `pendingChart` + opens `PcPairingModal` (instead of the canvas); `sendToTablet` sends `pendingChart.dataUrl` (raster fallback) + `pendingChart.fabricJson` (object-level) when re-editing, else the blank template; `onSaved` funnels through the EXISTING `handleSave` (editingIdx≥0 → replace the same slot); `editHere`/`closePairing` clear both pendings. Edit/delete buttons gained `data-testid`s.
- **Reuses** `serializeFabricCanvas` / `isObjectLevelReeditable` / `uploadTransportJson` / `downloadTransportJson` (DRY, Rule of 3) — **no new collection, no new storage rule** (the §followup-2 `uploads/chart-edit-sessions/{sessionId}/{file=**}` json allowance already covers `edit.json`).

**Graceful degradation**: a legacy chart with no `fabricJson`, OR the pre-storage-deploy window where the CLIENT `edit.json` upload is denied → `editFabricJsonUrl` stays null → the tablet uses the annotated PNG as a locked background (raster, add-on-top). Object-level unlocks after `firebase deploy --only storage` (#13).

**Verification (Rule Q V66 + Rule S)**:
- **Full vitest 13965/0** + build clean (3.43s).
- **L2 e2e ALL PASS on real prod** — `scripts/e2e-chart-relay-roundtrip.mjs` NEW Phase E: PC ships the chart PNG + `edit.json`; the session doc carries the NEW `editFabricJsonUrl`; the round-tripped `edit.json` passes the **PRODUCTION `isObjectLevelReeditable`** predicate (imported from `tabletChartTools.js`, NOT a hand-rolled mirror — V66); the canvas dims (600×800) survive the round-trip; the prior annotations ("EXISTING" Textbox) are present; the re-edited result merges back into the SAME slot (8→9 objects). Plus the existing A/B/C1-C4 still green; cleanup zero orphans. (Admin SDK bypasses the storage-rule gate, so this proves the data contract regardless of the deploy state.)
- **L1 real-browser** (Rule S, temp probe `src/__reeditL1Probe.jsx` deleted after): mounted the REAL `TabletChartCanvas` with a real-fabric `initialFabricJson` (built via the production `serializeFabricCanvas`) + empty `templateImageUrl` → read `exportFabricJson()`: `exportObjects:2` (loadFromJSON ran, the prior Rect+Textbox loaded as objects) + `exportW/H:600×800` (sized to the json's NATIVE dims via `setDimensions`, NOT the ~618 container-fit raster fallback) → the object-level hydrate path is PROVEN on a real mounted component with real fabric — exactly what source-grep + RTL-mocks cannot show.
- `tests/re-edit-chart-on-tablet.test.jsx` RT1-RT7 (RT1 buildSessionCreate field unit / RT2-RT4+RT7 source-grep / RT5 ChartSection RTL: ✏️ opens the modal not the canvas + edit-here opens the canvas / RT6 Rule I flow: handleEdit→onSaved→handleSave replaces the same slot via the REAL ChartSection wiring).

**V21 fixups (Rule P — the full-suite sweep caught 3 sibling locks of shapes this feature changed)**: `RC2` (more-tools flow-simulate — the template effect changed from a one-liner to the gated form) + `R4.2` (transport — ChartSection `templateDataUrl` now conditional on `pendingChart`) + `R4.4` (transport — openSession late-arrival moved into `resolveSource`). Each updated to lock the NEW contract + document the transition. (Same V21 family as every prior tablet-chart round; the full-suite run IS the cross-file grep.)

**Lessons**:
- **Threading one field through a proven relay + reusing the existing transport/serializer beats new infra** — the entire feature is `editFabricJsonUrl` + an `initialFabricJson` prop + a `pendingChart` state; the relay, transport, serializer, and storage rule were all already there. Cheaper to build, cheaper to verify, lower blast radius.
- **Fabric object coords are absolute → a re-edit MUST recreate the saved-dims canvas** — `serializeFabricCanvas` embeds `canvasWidth/Height`; `hydrateFromJson` `setDimensions` to them before `loadFromJSON` (the §followup-5 PC lesson, applied to the tablet).
- **No-double-load is a two-part gate** — the page resolves json-FIRST (skips the raster PNG when object-level), AND the canvas's `[templateImageUrl]` effect early-returns when a reeditable json owns it. Either alone would leave a flash / a double-baked image.
- **Rule Q V66 honored** — L1 real-browser proved the mounted-component object-level path (the riskiest, untested-by-mocks behavior); L2 used the PRODUCTION predicate against the real round-tripped json (no fixture divergence). The test bank verifies WIRING; L1/L2 verify BEHAVIOR.
- **Object-level tablet re-edit is live-gated on the storage deploy** (client `edit.json` upload denied until then → raster fallback, which works) — same gate as the §followup-5 PC object-level re-edit. NOT deployed (await "deploy"); post-deploy on-device L1 by the user.

#### §followup-7 — Arrow removed-on-release + text trapped-in-editing (2 tool bugs; /systematic-debugging) (2026-05-21 EOD+2)

User report (verbatim): "tools ที่เป็นหัวลูกศรใช้ไม่ได้ ขึ้นรูปลูกศร แสดงใน canvas แต่ปล่อยเม้าแล้วไม่วาดลงใน canvas" (arrow shows during drag but isn't drawn on release) + "tools ที่ใช้สร้าง ตัวอักษร ไม่สามารถกำหนดความกว้างของ box … ไม่มีที่ให้เลื่อนปรับขนาด…และตำแหน่งเลย" (text box: can't set width / no resize+move handles). Both in the more-tools `TabletChartCanvas` (local, not deployed). `/systematic-debugging` — Iron Law: root cause before any fix.

**Phase 1 (root cause + real-browser repro).** Bug 1 root-caused by reading: `commitShape` computed the "tiny"-discard `dist = Math.hypot((o.x2 ?? 0) - (o.x1 ?? 0), …)`, but the arrow `o` is a `fabric.Group` (buildArrow) with NO `x1/x2/y1/y2` → all `undefined` → `dist = 0` → `0 < 4` → `tiny = true` → `fc.remove(o)` on mouse:up. The arrow renders during drag (updateShape) then is removed on release — EXACTLY the symptom. `line` works (real `fabric.Line` with x1..y2). Bug 2 needed observation. **Real-browser probe** (temp `src/__toolProbe.jsx` + a temp `__fc` accessor on the imperative handle — both removed after): mounted the REAL component + drove the actual `fc.on('mouse:*')` handlers via **`fc.fire`** (bypasses the synthetic-event `isTrusted` limit — fire invokes the JS listeners directly). Evidence: arrow `objs 1/hasArrow true` during drag → `objs 0/hasArrow false` after release (Bug 1 confirmed); text `isEditing:true, hasControls:false` immediately after create, but after exit-editing+select `selectable:true` + all 9 controls positioned (Bug 2 = the auto-enter-editing traps the user with no handles). (`lineCommitted/rectCommitted:false` in the first run were a probe bug — fabric v7 instance `.type` is lowercase `'line'/'rect'`, my check used PascalCase; corrected → both true.)

**Phase 2 (compare to working reference).** Bug 1: rect/circle commit via `o.width/height`, line via `o.x1..y2` — correct for each type; the arrow Group is the lone shape mis-routed through the Line-geometry path. **Class-of-bug**: a polymorphic shape set must measure the GESTURE, not introspect type-specific object geometry. Bug 2: the proven PC `ChartCanvas` text does NOT auto-enter editing — it adds + `setActiveObject` + `setTool('select')`, leaving the text selected with handles (double-click to edit). The tablet `addText` had diverged with `setTimeout(enterEditing+selectAll)`. Both isolated 1-instance (cross-file grep: `commitShape`/`addText` are the only sites).

**Phase 4 (fix + verify).** (1) `updateShape` tracks the drag end `s.ex/s.ey`; `commitShape` line/arrow tiny-check uses the drag delta `Math.hypot((s.ex ?? s.sx) - s.sx, (s.ey ?? s.sy) - s.sy)` — geometry-agnostic (Line AND arrow Group). (2) `addText` mirrors the PC: `tb.set({selectable,evented})` + `setActiveObject` + switch to select, NO auto-edit — the text is created SELECTED with handles; double-tap to edit; the ml/mr handles set the box width. **Re-verified L1 (real browser, fc.fire) post-fix**: arrow `objs 1/hasArrow true` after release (survives); line/rect `true/true`; text `isEditing:false, hasControls:true, selectable:true` + 9 controls positioned. **COMPREHENSIVE all-tools L1 sweep (user demand "ทดสอบมาทุก tools")**: a 2nd probe drove EVERY tool through the real handlers — pen ✓ / highlighter ✓ (opacity 0.4) / line ✓ / **arrow ✓ (group)** / rect ✓ / circle ✓ (ellipse) / **text ✓ (isEditing:false, hasControls:true)** / eraser ✓ (removes) / **select ✓ (selectable + 9 control handles → move/resize)** + `tinyRectDiscarded`+`tinyArrowDiscarded` true (accidental taps still discarded — the drag-delta fix preserves that for the arrow too). (The pixel screenshot timed out — a harness renderer hiccup with the full-screen probe host — but these are object-LIFECYCLE fixes whose on-screen rendering rides the already-pixel-verified §followup-3 sync-render + §followup-4 cover fixes; no console errors.) Temp probes + `__fc` removed. Regression: `tests/tablet-chart-tool-bugs.test.jsx` TB1 (drag-delta + anti-regression on `Math.hypot((o.x2`) + TB2 (selectable-with-handles + anti-regression on `enterEditing`/`selectAll`). **AV106** locks both. **Comprehensive green**: full vitest **13970/0** (an earlier "1 failed" was a flake — non-reproducible on re-run) + 18 chart-system files/147 tests + L1 all-tools + **L2 e2e ALL PASS on real prod** (relay + persist + re-edit + Phase E + stress) + build clean.

**Lessons**: (a) **a polymorphic shape set must measure the gesture, not introspect type-specific object geometry** — the arrow Group has no Line props; the drag-delta is the type-agnostic measure. (b) **don't auto-enter editing on a freshly-created text object** — editing mode hides the resize/move controls (`hasControls=false`); leave it selected-with-handles (mirror the proven reference), double-tap to edit. (c) **`fc.fire` drives the real handlers in a real browser, bypassing the synthetic-event `isTrusted` limit** — the right way to L1-verify mounted-component pointer logic the harness otherwise can't drive (synthetic `dispatchEvent` is `isTrusted:false` → Fabric ignores it). (d) **fabric v7 instance `.type` is lowercase** ('line'/'rect'/'textbox'); only `toJSON().type` is PascalCase — a probe/test inspecting the instance must use lowercase (a false-negative bit me mid-probe). (e) **a source-grep anti-regression must target the CODE pattern, not a bare word** that can appear in a comment — `not.toMatch(/enterEditing/)` matched my own "do NOT auto-enterEditing" comment; reworded the comment to drop the literal.

---

### V122 — 2026-05-26/27 — Backup/restore SUBSYSTEM down: Vercel 300s timeout (sequential I/O) + 28-collection scope drift (AV141 + AV142)

User report (with screenshot of "จัดการไฟล์สำรองข้อมูลทั้งหมด"): the **Whole-System Backups (V81)** had been silently broken for days — `auto-20260521-0300` healthy (5,604 docs / 379 users / manifest) but **every backup since `auto-20260522-0300` showed `⚠ NO_MANIFEST` (0 docs / 0 users, ~7-8 MB partial)**. User: "ระบบ Backup บางอย่างอาจจะพังมาหลายวันแล้ว อาจจะเพราะเราอัพเดทโครงสร้างเยอะ … ฝากดูระบบ backup อื่นๆด้วย ทั้ง backup สาขาด้วย … dump ออกมาแล้ว restore เข้าไปได้แบบ 100% ทุกรูปแบบการ backup … e2e, stimulate flow, stress … ระบบ Backup สำคัญมากๆต่อธุรกิจ". `/systematic-debugging` + "ultrathink".

#### Root cause (CONFIRMED, not deduced — Rule R + Rule Q)

Two distinct bugs, one class:

**Bug 1 — the crash (NO_MANIFEST since 05-22).** The V81 backup executor performs **~1,000+ SEQUENTIAL network round-trips** per run: per-collection `.get()` + `storage.save()`, 107 customers × 8 subcollections = 856 `.get()` (most empty, all read), chat_conversations × messages, per-storage-file copy + getMetadata + full-byte-download SHA-256, auth, manifest. On Vercel (region `sin1`) the cross-region latency × that many sequential awaits **exceeds the 300s `maxDuration` cap** → the function is KILLED mid-flight (after collections, before the auth+manifest writes) → folder has files but NO manifest.json → `NO_MANIFEST`. The kill is process-level (bypasses ALL per-item try/catch) → the **day-to-day VARIATION in the death point** (subcoll files 0/3/5; auth always absent) is the timeout-near-boundary signature. 05-21 squeaked under 300s; data growth tipped 05-22+ over. Executor code was UNCHANGED since 2026-05-17 (V81-fix6) → not a code regression → pure data/scale-vs-cap.

  - **DECISIVE EVIDENCE**: 3 read-only Rule-R diags + 1 real trigger.
    - `diag-whole-system-backup-failure.mjs`: folder forensics — healthy 05-21 has `auth/users.json` + manifest; ALL broken folders have universal 23/23 + branch 30/30 but `auth/users.json`=0 + storage/=0 + no manifest (died around step 6/auth). Step-7 storage workload only ~6s (NOT the bottleneck — first hypothesis killed).
    - `diag-whole-system-backup-timing.mjs`: local read-sequence ≈ 25s (107 customers, not the "379" which was AUTH users) — far under 300s LOCALLY, proving the multiplier is Vercel↔Firestore cross-region latency, not the read count per se.
    - `diag-trigger-whole-system-backup.mjs`: **the deployed manual endpoint returned HTTP 504 `FUNCTION_INVOCATION_TIMEOUT` after 300.7s** + a **20h-stale cron lock** (its `finally{}` never ran = killed mid-flight). Definitive.

**Bug 2 — silent incompleteness (even "healthy" backups).** The backup scope was HARDCODED lists (`UNIVERSAL_COLLECTIONS` + `BRANCH_SCOPED_COLLECTIONS`). Prod had **65 data collections; 28 were silently OMITTED**, incl. MONEY (`be_deposits`, `be_wallet_transactions` 260, `be_point_transactions` 231), INV/HN COUNTERS (`be_customer_counter`, `be_sales_counter`), master data (`be_master_products` 275, `be_master_courses` 110, …), `be_course_changes`, `clinic_schedules`, `be_appointment_slots`, `be_recall_cases`, charts. A restore from even the "healthy" 05-21 backup would have LOST all of it. Same drift family as AV138 (every client collection needs a rule): a hardcoded "all collections" registry rots as features add collections.

**The class spans 4 families** (Rule P audit): whole-system (broken), customer-only (same executor), whole-fleet RESTORE (uncapped sequential per-customer loop), branch V40 (T4 customer-scan against a tighter 60s cap). The RESTORE executor has the identical sequential pattern — and Replace-mode restore calls the slow backup executor as its auto-pre-backup → guaranteed timeout.

#### Fix (V122 — one coherent change across the class)

1. **NEW pure helper `mapWithConcurrency(items, limit, fn)`** (wholeSystemBackupCore.js) — bounded-parallel, index-stable order, error-propagating. Plus `classifyCollectionCategory` + `FULL_SCOPE_COLLECTION_DENYLIST`.
2. **`wholeSystemBackupExecutor.js`**: parallelized collection (×20) / subcoll (×40) / chat (×40) / storage (×15) I/O + **DYNAMIC `db.doc(PREFIX).listCollections()` enumeration** for full scope (closes the 28-collection drift forever; customer-only keeps its curated subset). ~1,000 sequential trips → ~50 batches (≈20× faster).
3. **`wholeSystemRestoreExecutor.js`**: parallelized `restoreCollections` / `restoreStorage` / `wipeFirebase` (subcoll + collection wipes) + dynamic enumeration for wipe + assertTargetEmpty. **Plus fixed a LATENT orphan-subcollection wipe-order bug**: the pre-V122 wipe deleted the `be_customers` collection BEFORE the subcollection wipe, so `be_customers.get()` was empty by then → customer subcollections were orphaned (Firestore does not cascade-delete subcollections) → stale subcoll survived a "full replace". Fix: capture doc refs via `listDocuments()` UP FRONT, wipe subcoll → then parent.
4. **`whole-fleet-customer-restore.js`**: parallelized the download-heavy per-customer LOAD phase (Phase A); the decision+restore loop stays SEQUENTIAL to preserve V77-fix2 intra-batch HN-collision ordering (`restoredLive.push`).
5. **`vercel.json`**: branch backup family `maxDuration` 60→300 (safety margin; user flagged branch backup).

#### Verification (Rule Q — honest L2 + structural + gap disclosure)

- **REAL-PROD dump→restore e2e** (`scripts/e2e-whole-system-backup-restore-v122.mjs`, **10/0**): fixed executor backed up in **7.5s** (vs 300s timeout), **0 failed collections**, manifest validates, **ALL 65 live collections captured** (money/counters incl.), Timestamp markers present. Restore round-trip into an **isolated namespace** (`data-V122-VERIFY-{ts}`, zero prod-data touched, cleaned up) — every doc **byte-identical** to live source (be_customers 107 / be_deposits 12 / be_wallet_transactions 260 / be_recalls 1 / chat_history 370), incl. Timestamp `.toMillis()` fidelity. (First run "all DIFFER" was a verification-script bug — order-sensitive `JSON.stringify` vs Firestore's non-guaranteed read order; canonical/sorted compare → 10/0. Rule Q-honest: investigated the diff, didn't assume.)
- **Branch family healthy** (`diag-prod-backup-full-validate.mjs`, after fixing its stale `schemaVersion!==1` assertion → accept 1|2): full backup 27.89s + auto-pre-fresh 28.19s + make-fresh precondition, all 200, valid, cleaned up. Branch backup was NOT broken; the maxDuration bump is insurance.
- **Tests**: `tests/v122-backup-parallel-and-completeness.test.js` 23/0 (mapWithConcurrency order/bound/10k-stress + classify + adversarial Timestamp/GeoPoint/Bytes/Thai/emoji round-trip + source-grep locks + completeness contract). Existing V81 57/0. **3 V21 fixups** (tests that locked the OLD `continue`-based / 4-encode-site shape → updated to the V122 `.filter`/`return false` / 3-site shape, same intent): v81-source-grep wipe-Storage + wipe-Firestore skips, v81-fix1 J.3 (4→3 encode sites). Build + `node --check` clean.
- **AV141** (serverless backup/restore I/O MUST be bounded-parallel, not sequential vs the 300s cap) + **AV142** (whole-system scope MUST be dynamically enumerated, no hardcoded-list drift) added to audit-anti-vibe-code.
- **HONEST GAP (Rule Q)**: local proves completeness + restore fidelity + speed; the literal **300s-relief on PROD** is proven *structurally* (20× fewer sequential trips → ~15s) — the DEFINITIVE "no more 504" requires **deploy + endpoint re-test** (`diag-trigger-whole-system-backup.mjs`), which is **gated on the user's explicit "deploy"**. The literal `runWholeSystemRestore` against prod is destructive (Fresh refuses non-empty / Replace wipes) → verified via the faithful isolated-namespace round-trip + pure tests, NOT against live prod data.

#### Lessons (locked permanent)

- (a) **A serverless function that does per-item I/O scaling with data WILL eventually hit the maxDuration cap** — 300s is a HARD ceiling you cannot raise; the fix is bounded-parallel I/O, not a bigger timeout. `mapWithConcurrency` is the canonical pattern; every future bulk/cron/backup endpoint should use it.
- (b) **A "back up everything" scope MUST be dynamically enumerated** (`listCollections()`), never a hardcoded list — the list rots silently as features add collections (28 omitted incl. money). Same family as AV138.
- (c) **Process-kill (timeout/OOM) bypasses ALL try/catch** — a backup whose per-step error handling is "tolerant" can STILL produce a NO_MANIFEST if the process is killed; the manifest-write being the LAST step means any kill → no manifest. The day-to-day death-point VARIATION is the timeout signature (vs a deterministic throw).
- (d) **Local admin-SDK runs CANNOT reproduce a cross-region prod timeout** (local latency is low) — they prove logic/completeness/fidelity, NOT the 300s relief. Be honest about that gap; the deployed-endpoint 504 (and its post-deploy re-test) is the only real proof.
- (e) **Rule Q-honest on the verification SCRIPT too**: the first e2e showed "all docs DIFFER" — I investigated (order-sensitive stringify vs Firestore's non-guaranteed field order) instead of assuming a real bug OR rubber-stamping; canonical compare proved byte-identical. A failing check that turns out to be a harness bug must be PROVEN so, not waved away.
- (f) **Firestore does NOT cascade-delete subcollections** — a wipe must delete subcollections BEFORE (or independent of) the parent docs, capturing refs via `listDocuments()` up front (catches phantom parents too). The pre-V122 restore-wipe order was a latent restore-fidelity bug.
