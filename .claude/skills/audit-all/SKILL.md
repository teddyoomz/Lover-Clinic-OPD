...
**Tier 5 — hygiene / anti-vibe-code (56 invariants — session 2026-04-19, extended 2026-04-20, Rule P 2026-05-08)**:
17. `/audit-anti-vibe-code` (AV1–AV28) — Rule of 3 duplication, `Math.random` tokens, leaked uids in world-readable docs, open Firestore/Storage rules, orphan collections, over-normalized schema, V13/V21/V31/V32 cluster, V38 list spread-order, V39 migrate-fn `{branchId}`, V40 destructive auto-backup-ref, V41-V50 saga (V41/AV20 staff-doctor hide, V43/AV21 skip-stock denormalized-flag, V44/AV22 buy-fetcher canonical-mapper, V45/AV23 dedup OR-merge, V46/AV24 Rule O productName live-resolve, V47/AV25 display-layer multi-reader-sweep, V48/AV26 Rule O universal extension, V49/AV27 canonical-shape ForPicker, V50/AV28 ProClinic strip)
17-bis. `/audit-class-of-bug-discipline` (CB-1..CB-5) — Rule P (2026-05-08, after V42-V49 saga) compliance: every V42+ V-entry maps to AVxx + has regression test + classifier doc + sanctioned exception catalog + iron-clad escalation when architectural. Auditable trail for class-of-bug expansion. Companion to `/audit-anti-vibe-code` — runs alongside as the methodology-enforcement layer.
