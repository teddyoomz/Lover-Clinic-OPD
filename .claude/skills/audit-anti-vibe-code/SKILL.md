...
### AV208 — TFP entry SWR + full-scan classification + cache-cap sizing (2026-07-18)

TreatmentFormPage's first paint was 100% network-gated — a ~600-doc / ~630KB 6-collection server pull on EVERY open — because TFP escaped the AV206 sweep (the classifier only checked files already in the ADOPT list). On clinic WiFi that pull hung the spinner 10-60s; heavy-use machines were WORST because the staff working set (~44MB in IndexedDB) sat AT the 40MB default cache cap → LRU evicted TFP's query targets between opens → every reopen was a cold pull. Three locks: **(1) TFP is an ADOPT surface** — its load effect runs `fetchFormData(source)` (FETCH-ONLY; cache pass returns null = MISS on empty lists / customer-absent / edit-treatment-absent — never a false paint) + `applyFormData` (setState; hydration + prefill ONCE across passes so the server pass never clobbers typing) orchestrated by `swrRun`; `<SyncIndicator/>` chip ON while cache data is unconfirmed (a network-down __fromCache server leg keeps it ON — honesty); handleSubmit awaits `serverFreshRef` bounded 15s (save-gate Q2=A) before serializing money/stock writes (belt on top of Rule T server txs). **(2) Full-scan classification** — the classifier walks ALL of src/components + src/pages for `await Promise.all([list*...])` mount-load shapes; every match MUST be classified in `docs/perf/swr-inventory.md` (ADOPT or SANCTIONED) — a page can no longer hide by not being listed (first run caught TFP + 5 form modals + 3 small tabs). **(3) Cache cap sized to the working set** — `cacheSizeBytes: 200 * 1024 * 1024` in src/firebase.js (NOT unlimited — GC stays as a multi-year backstop); re-check with `scripts/diag-staffapp-working-set.mjs` when the schema/data footprint grows. **Grep**: TFP must contain `swrRun(` + `serverFreshRef` + the 3 cache-MISS gates; firebase.js must contain `cacheSizeBytes: 200 * 1024 * 1024`; any new `Promise.all([list*` surface not in the inventory fails the scan. **Verified**: tests/tfp-entry-swr-contract.test.js (C1-C6) + tests/tfp-entry-swr-flow-simulate.test.js (F1-F7, REAL swrRun) + tests/tfp-entry-swr-getters.test.js (G1-G4) + tests/tfp-prefetch.test.js (P1-P5) + the AV208 block in tests/instant-coldstart-av206-classifier.test.js + real-prod probes scripts/diag-tfp-{entry-payload,entry-probe,cold-timeline,idbcold-probe}.mjs + diag-staffapp-working-set.mjs.

### AV209 — customer.courses[] row targeting is IDENTITY-FIRST; a UI-frozen index is only a hint (2026-07-18)

The positional-rowId TOCTOU class: CustomerDetailView (`entry.originalIndex`) and RemainingCourseTab (`row.courseIndex`) freeze an array index at render, then commit minutes later — a concurrent insert/remove/reorder of `customer.courses[]` from another machine makes the stale index target the WRONG row inside the tx (wrong course adjusted / exchanged / refunded / cancelled, silently — money-adjacent). One mechanism closes the class: `resolveCourseRowIndex` (src/lib/courseExchange.js) — `courseId` wins; the index hint is accepted only when the row at it still matches the supplied `name`/`product`; else an UNAMBIGUOUS identity search; else -1 → `COURSE_ROW_STALE_MSG` (Thai refresh error — never guess among duplicates). Mirrors the proven `matchesDed` hint-then-validate pattern of `deductCourseItems`. **Rules**: (1) every courses[] mutator that accepts a `courseIndex` (`adjustCourseRemainingQty`, `exchangeCourseProduct`, `applyCourseRefund`, `applyCourseCancel`, `removeCustomerCourseRowAtomic`) MUST resolve via `resolveCourseRowIndex` INSIDE the tx — a bare bounds check is FORBIDDEN as the sole guard when the caller can supply identity; (2) every UI callsite that passes a `courseIndex` MUST also pass the row's identity (`expectedName`/`expectedProduct`, + real `courseId` when the row has one); `deductCourseItems` deduction objects MUST carry `productName` alongside `courseName` (name-only match let a same-name/different-product row absorb the deduction); (3) NO `getCustomer → mutate courses → updateCustomer({courses})` whole-array RMW outside a tx (Rule T; single-row removal goes through `removeCustomerCourseRowAtomic`). Legacy identity-less callers keep bounds-only behavior — but NEW callsites without identity fail review. **Hunt-loop hardenings (2026-07-19, R1→R3 converged)**: (R1) a STRING identity value is a CONSTRAINT INCLUDING `''` (legacy ''-product rows never match a different-product sibling; `undefined` = no constraint — never coerce with `|| ''` in the threading); a supplied courseId that no longer exists = DEFINITIVE staleness (hard -1 — an identity twin of the deleted purchase must never be fallen into). (R2) terminal-row handling is SPLIT: an identity-MATCHING row at the INDEX HINT is returned even when terminal (in-place terminalization — downstream raises the informative error; redirecting to a live twin silently mutated a DIFFERENT purchase), while the identity SEARCH excludes terminal twins (array-shift case); `adjustCourseRemainingQty`/`exchangeCourseProduct` throw `COURSE_ROW_TERMINAL_MSG` and `removeCustomerCourseRowAtomic` no-ops on a terminal target (shared `isTerminalCourseRow`). (R3) `applyCourseRefund` refuses a CANCELLED row (`course already cancelled`) — the sale-cancel cascade already reversed the money; a refund would stamp a double-reimbursement record. Known irreducible tail: a SPLICED legacy row (no courseId) with exactly one live identity twin resolves to the twin — indistinguishable by name+product; modern rows are covered by the courseId hard-fail. **Grep**: `adjustCourseRemainingQty(`/`exchangeCourseProduct(`/`refundCustomerCourse(`/`cancelCustomerCourse(` callsites in src/components must show `expectedName`; `updateCustomer\([^)]*\{ courses:` in components = violation; `expectedName: [^,)]*\|\| ''` in the wrappers = violation (undefined must flow through). **Verified**: tests/av209-course-row-identity.test.js (A pure matrix incl. A11-A15 hunt locks + B/C/E execution with only the tx boundary mocked + D refund/cancel + F source-grep locks) + scripts/e2e-av209-course-row-identity.mjs (Rule Q L2, real prod, re-run after every hunt round).

**Follow-up (2026-07-19 EOD+1) — the irreducible tail is CLOSED**: (a) every course-row WRITER now stamps a unique per-ROW `courseId` — `assignCourseToCustomer` per-product + no-products branches and `resolvePickedCourseInCustomer` resolved siblings stamp `crs-<ts>-<i>-<rand>`; `addPicksToResolvedGroup` STRIPS the template sibling's courseId from the spread (`courseId: _stripCourseId`) + stamps fresh (a spread-copied id = duplicate ids = ambiguous byId). Namespace `crs-`/`crsbf-` is disjoint from `purchased-`/`promo-`/`pick-`/`exchange-`/`be-course-`/`legacy-`/`idx-` sentinels so no rowId-contract filter can misroute. (b) Rule M backfill `scripts/av209-backfill-course-row-courseid.mjs` stamped `crsbf-` on ALL 523 identity-less prod rows across 123 customers (2026-07-19, audit `av209-courseid-backfill-*`; per-doc runTransaction vs live writers; idempotent re-run = 0). **Rule**: any NEW courses[] row-creating writer MUST stamp a unique per-row courseId; inheriting a sibling/template courseId through an object spread is FORBIDDEN. **Grep**: every `courses.push({`/`resolvedEntries`/`newEntries` builder in backendClient.js must contain `courseId: \`crs-` OR a `pick-` stamp; `\.\.\.baseTpl` without a `courseId: _strip` destructure = violation. **Verified**: tests/av209-course-row-courseid-stamp.test.js (E1-E6 execution + SG1-SG3 + D1-D4 backfill helper) + scripts/diag-av209-backfill-verify.mjs (real prod rows resolve byId to their own index).

### AV210 — security-response-header changes MUST be audited against every NON-PAGE consumer; SW-path CSP must allowlist its importScripts origins (2026-07-19)

The header-vs-consumer drift class: WS4 (`d48f79b6`, 2026-06-10) added a site-wide CSP written against the PAGE's needs (inline-script hashes, googleapis connect-src) — nobody audited the SERVICE WORKER, whose first line is `importScripts('https://www.gstatic.com/firebasejs/...')`. A SW's evaluation obeys the CSP delivered WITH ITS OWN response → `script-src 'self'+hashes` blocked the import. **Latent for 27 days** because already-installed SWs never re-evaluate; AV207 (2026-07-07) moved the FCM SW to its own scope → every device needed a FRESH registration → evaluation failed fleet-wide ("ServiceWorker script evaluation failed" in Chrome; "**NetworkError: A network error occurred.**" in WebKit/iOS — a misleading error that reads as connectivity). Compound damage: the app-shell sw.js that replaced the old scope-'/' registration INHERITED its live push subscription but has NO push handler → FCM sends to all 8 legacy tokens returned SUCCESS while displaying NOTHING → the sender's not-registered prune never fired → push silently dead 07-07→07-19 with zero errors anywhere. **Rules**: (1) any change to response headers that carry an execution policy (CSP / COEP / CORP) MUST enumerate NON-page consumers — service workers (importScripts!), web workers, manifest, notification icons — and be verified against each; (2) the CSP served on a SW script path MUST allowlist every `importScripts` origin of that script (dedicated per-path header rule in vercel.json, placed AFTER the global rule — Vercel later-wins); (3) the PAGE-global script-src must NOT be widened to fix a worker (www.gstatic.com hosts known CSP-bypass gadgets — old AngularJS); (4) every push-token mint site MUST clean up the legacy root-scope subscription AFTER minting (`cleanupLegacyRootPushSubscription` in AdminDashboard — a handler-less SW holding a live subscription is an invisible sinkhole that defeats FCM's own token pruning); (5) "FCM send success" is DELIVERY-ACCEPTED, not DISPLAYED — display verification needs a real device (Rule Q L1; `scripts/diag-push-test-send.mjs`). **Grep**: `importScripts\(['"]https?://` in public/ → each origin must appear in the `/firebase-messaging-sw.js` headers rule's script-src; global `/(.*)` script-src must NOT contain `gstatic.com`. **Verified**: tests/av210-push-csp-sw-consistency.test.js (C1-C7) + scripts/prune-legacy-push-tokens.mjs (Rule M) + scripts/diag-push-test-send.mjs (post-deploy real-send).

### AV211 — every scheduled cron MUST declare its health-monitor coverage; background failures MUST be observable (2026-07-19)

The silent-infra-death class: push outage silent 12 days (AV210) · whole-system backup NO_MANIFEST silent 5 days (V122) · chat-history retention cron dead for 46 runs (Timestamp-type mismatch) — every one invisible until a human stumbled on it, because crons/push/backup had no observer. The infra-health-sweep cron (daily 07:30 BKK) now watches every liveness surface (`clinic_settings/scheduled_task_status` + recon deterministic doc + `push_config/tokens` freshness + `client_error_log` volume) via the pure evaluator in `src/lib/infraHealthCore.js` and alerts through TWO FCM-independent channels (staff-chat system card kind `infra-health` + LINE OA text push — FCM cannot announce its own death). **Rules**: (1) every path added to `vercel.json crons[]` MUST be declared in `INFRA_TASK_EXPECTATIONS` (with `maxAgeHours` + severity) OR `INFRA_SPECIAL_CRON_PATHS` OR — explicitly and with a written reason — `INFRA_UNMONITORED_CRON_PATHS`; the classifier test turns RED otherwise; (2) a new cron that writes `scheduled_task_status` gets liveness coverage for free — prefer that rail (`writeScheduledTaskStatus`); (3) alert channels for infra failures must not depend on the subsystem being monitored (no FCM-push alert for "FCM is dead"); (4) `client_error_log` is endpoint-only (default-deny; beacon writes via `api/client-error.js`, admin reads via `api/admin/client-errors-list.js`) — no client-SDK reference to it may appear in `src/lib` data layers; (5) beacon payloads carry query-param NAMES only, never values (patient link tokens travel in query values — PHI/token hygiene). **Grep**: parse `vercel.json` crons[] → assert ⊆ declared sets (tests/infra-health-core.test.js H4); `client_error_log` in src/lib/backendClient.js + scopedDataLayer.js → must be absent (tests/infra-health-flow-simulate.test.js F6.4). **Verified**: tests/infra-health-core.test.js + tests/infra-health-flow-simulate.test.js + scripts/diag-infra-health.mjs (Rule R read-only vs real prod) + scripts/e2e-client-error-endpoint.mjs (Rule Q L2 vs real prod).

### AV212 — the app must SURVIVE degraded machines: guarded IndexedDB, retry-wrapped lazy chunks, parallel SWR legs, staged load escapes, telemetry over guesswork (2026-07-20)

The degraded-machine class, found by the 14-cell degradation matrix (`tests/e2e/machine-degradation-matrix.spec.js` — CPU ×6/×20, 400kbps/offline nets, IDB absent/broken/quota-starved, warm/cold, vs the LIVE bundle): **(M7)** an IndexedDB whose `open()` THROWS (corrupt Chrome profile / disk-full / AV interference) tripped `FIRESTORE INTERNAL ASSERTION (ID: b815)` → the whole React tree crashed — the SDK's memory fallback only covers the async open-error path; **(M10)** a lazy chunk fetched while offline rejected `React.lazy` → AppErrorBoundary replaced the ENTIRE app ("Failed to fetch dynamically imported module") — 79 lazy sites shared the class; **(M12)** a weak-CPU machine's entry is LEGIT-slow (35s at CPU×20, warm cache — IDB read/write amplification dominates, NOT network) so a 15s retry button invited doom-restarts of pulls that were 75% done. **Rules**: (1) `src/firebase.js` derives `canPersist` from the `idbHealthy()` pre-flight probe (sync-throw caught NOW; async open-error stamps `lover.idbBroken` so the NEXT load boots memory-cache — self-healing in one reload) — a bare `typeof indexedDB` check is FORBIDDEN; (2) every `indexedDB.open` in `src/` lives in the sanctioned guarded set {firebase.js, stickerLibrary.js} — new IDB consumers must guard absent AND throwing IDB; (3) every `lazy(() => import(...))` site routes through `lazyRetry` (host files alias `lazyRetry as lazy` — chunk failure = retry ×2 → in-place panel `chunk-load-retry`, NEVER a rejection into the boundary); (4) `swrRun` runs cache + server legs in PARALLEL (a slow/hung IDB read must not delay the server truth; late cache settle never paints over applied server data); (5) the TFP load escape is STAGED — 15s calm "กำลังโหลดต่อ" info, retry button only at 30s (`loadStuck`); (6) slow entries (>10s) + degraded environments report `kind:'telemetry'` beacons (bucketed messages, `envTelemetry.js`) which the infra-health errorCount EXCLUDES (a chronically slow machine must not trip the daily error alert) but the health-card viewer SHOWS (the "เครื่องไหนช้าเพราะอะไร" answer is data, not guesswork); (7) **TFP fast-paint pre-stage** (≤5s-เมื่อเน็ตโอเค directive): CREATE-mode entry paints from the SMALL reads only (doctors+staff+customer ≈15 docs) — the ~600-doc enrichment (products/courses/DF) loads behind the chip, because Firestore's local cache has NO indexes (every cache query UNPACKS all stored docs → the big lists cost 30-45s on weak CPUs even with zero network); the `fullApplied` flag makes a late fast-paint a no-op (never minimal-over-full), the save-gate still awaits the FULL pipeline (money serialization never reads the minimal subset), and edit-mode keeps full blocking (hydration deps); money actions (save/buy) gate on `optionsEnriched` (set ONLY by the full applyFormData; vitals save exempt — its path touches no money block) with the painted-form stuck-banner escape at 30s; (8) **slow-machine adaptive persistence** (the 10-year-laptop class — "สมัยแรกๆ เครื่องนี้เร็วปกติ" because the IDB was tiny then): the TFP fast-paint TIMES its pure-cache read attempts (network-free IDB probe); ≥2 of the last 3 probes >1500ms with persistence ON → `machinePerf` stamps `lover.noPersist` (14d TTL) → next boot = memory cache (matrix: M6 no-IDB 1.2s vs M12 warm-IDB ×20 = 14-35s — server pulls beat a grinding indexless IDB on weak hardware); manual override + local-cache wipe live in the 🩺 health card `infra-machine-box`; the decision is MEASURED per machine, never spec-guessed; (9) **server-side read model for the heavy lists** (`/api/tfp-options`, the /api/patient-view pattern): ONE authed request (isClinicStaff/admin claims — same gate as the firestore rules; `Cache-Control: private, no-store` because a shared CDN cache would serve authed master data to anon URL guessers; module-scope 30s memory cache instead) returns the 4 heavy lists byte-shape-identical to the client listers (`{...data, id}` + verbatim sort mirrors) → the client marries them with the fast-paint's real doctors/staff/customer and feeds the UNCHANGED `applyFormData` (single mapper — zero V12 drift) via the same applyChain, tagged `fromCache:true` (chip honesty) and guarded by `serverConfirmed` (a ≤30s-stale bundle never overwrites genuine Firestore server data); every failure path is a silent no-op (the SWR pipeline carries as before) — the client cost curve becomes O(payload), never O(IDB), which is the ten-year answer. **Grep**: `tests/machine-perf-ratchet.test.js` (R1-R3) + `tests/machine-degradation-hardening.test.js` F7-F10 + matrix cell M14; post-deploy L2 = `scripts/diag-tfp-options-endpoint.mjs`. **Grep**: `tests/machine-degradation-hardening.test.js` (A parallel-legs execution · B firebase probe · C lazyRetry universal classifier — any src file with `lazy(() => import` must alias lazyRetry · D telemetry kind allowlist + sweep exclusion · E unguarded-IDB classifier). **Verified**: the matrix e2e vs the LIVE prod bundle + local fixed build (M0 1.6s · M5 warm+400kbps 0.56s · M9 hell CPU×20+400k+no-IDB 11.9s no-card · M11 typing at ×20 = 88ms/key · M7/M10 crash→survive after fix) + RT bank staged-escape locks (tests/tfp-resilient-timeout.test.js).

### AV213 — LINE friend roster: admin-SDK-only writers, single-source real-time picker, mirror-approve binds (2026-07-20)

The LINE Friend Picker class (user pain: "หา user id ไม่ได้" — admins hand-typing `Uxxxx…` into infra-health LINE targets; customers typing เลขบัตร to self-link). `be_line_friends` (docId `${branchId}_${lineUserId}`) is the per-branch follower roster with EXACTLY two writers, both admin SDK: the webhook follow/unfollow handler (best-effort branch in `api/webhook/line.js` — placed AFTER config resolution, ALWAYS returns before the chat path, fallback branch via `resolveChatFallbackBranchId` per the V78 BUG-XR-24 lesson) and the `/api/admin/line-friends` Followers-API backfill (`action:'list'` — 403 from an unverified OA is `followersApi:'unavailable'`, NEVER an error; unknown-id-only profile resolves, `mapWithConcurrency` 10, cap 300, module-cache 60s/branch). **Rules**: (1) firestore.rules `be_line_friends` = `read: isClinicStaff / write: false` — the staff READ powers the real-time picker listener (แอดปุ๊ป/ทักปุ๊ป โผล่ปั๊บ, Rule B probe #20); any client-SDK write reference to the collection in `src/` is a violation; (2) the picker (`LineFriendPickerModal.jsx`, the ONE shared component for infra-health + customer-bind — V12 sibling-drift guard) renders from its TWO onSnapshot listeners ONLY (`listenToLineFriendsByBranch` BS-13 equality-only query — deliberately NO orderBy, so no composite-index dependency; sort is client-side in `mergeFriendRoster`) — the backfill endpoint FEEDS the listeners and must never become a parallel render path; (3) every roster merge goes through the pure `src/lib/lineFriendRoster.js` helpers (dedupe key = lineUserId; 'chat' badge wins; fresher-non-empty name wins; `unfollowedAt` is a soft flag that merges never clear except a re-follow); (4) picker-originated customer binds go ONLY through `/api/admin/line-friends action:'bind'` which mirrors link-requests `handleApprove` byte-for-byte on the write shape (collision guard `where('lineUserId','==',…).limit(2)` → Thai error + ZERO writes; atomic batch = customer `lineUserId`+`lineLinkedAt`+`lineUserId_byBranch.{branchId}` dotted-path + a `be_admin_audit/line-friend-bind-*` doc `source:'friend-picker'`; LINE push best-effort) — a client-side direct `updateCustomer` bind is FORBIDDEN; (5) follow-event capture must stay BEST-EFFORT (try/catch + warn + return) — a roster failure may never touch the chat message pipeline (V32-tris-ter ordering contract). **Grep**: `tests/line-friends-webhook-follow.test.js` (W1-W6 incl. rules shape + COLLECTION_MATRIX + probe #20 doc) + `tests/line-friends-listener-bsa.test.js` (L1-L4) + `tests/line-friends-endpoint.test.js` (E1-E7) + `tests/line-friend-picker-rtl.test.jsx` / `-integration-rtl.test.jsx` + `tests/line-friends-flow-simulate.test.js` (F1-F6). **Verified**: Rule Q L2 `scripts/e2e-line-friends-realtime.mjs` 16/0 vs REAL prod (realtime 61ms · client write+read denied pre-rules · REAL handleBind read-back · zero-orphan; `--full` re-proves the client-listener + live-HTTP legs post-rules-deploy) + Rule Q L1 `tests/e2e/line-friend-picker.spec.js` (a conv doc seeded MID-OPEN appears live, Q-vis screenshots) + `tests/e2e/hub-done-sort.spec.js` (the companion done-tab sort feature).

### AV214 — every shared heal path must be un-latchable (timeboxed) and every retry ladder must end in a hard reload (2026-07-20)

The wedged-client class, found by /systematic-debugging on the field report "มือถือสลับ frontend↔backend ค้าง → กดลองใหม่ไม่หาย → ต้องปิดแอปเข้าใหม่" (iPhone PWA, beacon log EMPTY = a silent hang — nothing threw). Root cause chain: iOS froze the background tab holding the `persistentMultipleTabManager` PRIMARY LEASE → every Firestore op on the foreground tab hung (cache reads INCLUDED) → `reconnectFirestore()`'s awaited `disableNetwork()` never settled → the module-level `toggling` latch stayed TRUE forever → every later heal path in the app (App.jsx V17 visibility/online, useResilientLoad auto-retry, manual ลองใหม่, useBranchAwareListener silent heal, TFP reconnect sites — ALL share the ONE latch) silently no-oped → the retry button could never work; only an app kill (fresh JS context + fresh lease) recovered. **Rules**: (1) any module-level in-flight latch around an awaited SDK operation MUST be timeboxed (`Promise.race` + `TOGGLE_TIMEOUT_MS`) so the latch provably always clears — an un-timeboxed `await` before `finally` is a permanent-wedge vector; (2) a timed-out toggle stamps the WEDGE marker (`isConnectionWedged()`) + `[conn-wedge]` telemetry beacon; a COMPLETED toggle clears it (the queue is proven alive); (3) every user-facing retry ladder must END in `hardReloadApp()` (the automated ปิดแอปเข้าใหม่ — heals every wedge flavor: frozen lease, dead async queue, poisoned SDK): `useResilientLoad.retry()` escalates when the client is wedged OR when a previous manual retry already failed; press#1 also fires `reconnectFirestore()` IMMEDIATELY (the press IS the broken signal — don't wait for the 8s soft-timeout); (4) `markReady` resets the escalation ladder (a recovered load must not leave a stale reload armed); (5) reloads are USER-INITIATED only (press) — no auto-reload loops; (6) the raced-out toggle promise carries a `.catch(()=>{})` so its late settlement never surfaces as an unhandled rejection (beacon noise). **Repro lesson (locked)**: an ABSENT or EMPTY-healthy IDB does NOT reproduce this class — cache legs answer instantly and `markReady` suppresses the banner (3 harness runs proved it); the faithful wedge = an IDB whose `open()` NEVER answers (no success, no error) for the `firestore*` database names. **Grep**: `tests/mobile-stuck-retry-escalation.test.jsx` (R1 timebox/wedge/latch-clears · R2 ladder · R3 Rule-I chain · R4 wiring locks incl. all consumer paths). **Verified**: Rule Q L1 `tests/e2e/mobile-stuck-retry-escalation.spec.js` — real browser, hanging-IDB wedge, pixel-faithful replica of the user's field screenshot (banner + counts 0 + กำลังโหลด) → press ลองใหม่ → a REAL `window.location.reload()` navigation observed.

### AV215 — a Firestore INTERNAL ASSERTION (ca9/b815) is an upstream-SDK wedge → route it into the AV214 ladder; self-healing chunk-load churn is telemetry, not an error (2026-07-23)

The infra-health class. The AV211 LINE alert fired 🟡 ("Client errors 24 ชม.: 7 รายการ", 07-23 07:30); the Rule-R beacon dump (`scripts/diag-client-errors.mjs`) showed the 7 = **4 benign lazy-chunk churn** (a staff tab on the pre-07-21 bundle requesting deleted chunk hashes — `lazyRetry`+SW self-heal) + **3 Firestore-stream** (2× `INTERNAL ASSERTION FAILED: Unexpected state` ID ca9→cascades-to-b815 + 1 `missing stream token`), one Win/Chrome session, already recovered by diagnosis time (live `errorCount24h:0`). Root cause confirmed against **firebase-js-sdk#9267** (OPEN, same ID + 12.x family): the assertion is triggered by repeated `disableNetwork`/`enableNetwork` churn (our `firestoreReconnect` toggle, many callers — telemetry `[conn-wedge] reconnect toggle timed out ×17`) + `persistentMultipleTabManager` — both documented triggers; the SDK bug is upstream/uneliminable, and once it fires the SDK state is corrupt (only a reload recovers). **Rules**: (1) pure `isFirestoreInternalAssertion(message)` (`clientErrorCore.js`) matches ONLY `INTERNAL ASSERTION FAILED: Unexpected state` (ca9/b815), NOT the transient `missing stream token` (the SDK self-recovers that — escalating on it is over-aggressive); (2) `reportErrorToBeacon` routes a match into a registered handler BEFORE the normal report (covers onerror / unhandledrejection / react-boundary — every path calls it); the handler SLOT (`setFirestoreAssertionHandler`) keeps the pre-boot firebase-free beacon out of the firebase/wedge import graph (no errorBeacon↔wedgeEscalation cycle) — `main.jsx` registers `onFirestoreAssertion` (firebase-FREE, in `wedgeEscalation.js`) right after `installErrorBeacon()`; (3) the handler REUSES the whole AV214 ladder (`escalateWedgeIfReloadFailed` → reachability probe → 1/hour cap → 24h `conn-wedge` TTL) and adds **NO new auto-reload** (AV214 invariant): first occurrence no-ops, a user-initiated reload stamps `WEDGE_RELOAD`, a recurrence within the heal window escalates to a memory-cache boot which DROPS `persistentMultipleTabManager` = removes one of the two triggers; the assertion STILL counts toward the health alert (it is a real crash); (4) `AppErrorBoundary.getDerivedStateFromError` flags an assertion crash so its reload button stamps `noteWedgeReload()` (render-crash path mirrors the onerror path); (5) DE-NOISE — `lazyRetry`'s exhausted chunk-load failure reports as `reportTelemetryToBeacon('[lazy-chunk] …')` (kind:'telemetry'), NEVER `reportErrorToBeacon` — a chunk-load failure is self-healing churn (in-loop retries + reload-fetches-a-fresh-index) that SPIKES on deploy days; the sweep's `errorCount24h` already excludes telemetry, so the daily alert is reserved for real runtime crashes and stops crying wolf every deploy. **Grep**: `tests/firestore-assertion-recovery.test.jsx` (A predicate · B beacon-routing-every-path · C onFirestoreAssertion ladder · D boundary reload-stamp · E full recovery flow · F wiring+de-noise locks; 21/0) + `scripts/diag-client-errors.mjs` (Rule R read-only dump). **Verified**: Rule Q L1 real browser (localhost dev, real boot-registered handler) — a dispatched `window.onerror` with the REAL ca9 string, after a stamped reload, escalates to `lover.noPersist` `reason:'conn-wedge'` 24h; a benign error / `missing stream token` / first-occurrence assertion do NOT escalate (precision). **Honest gap**: the SDK ca9 race is non-deterministic → cannot be reproduced in-browser; the true "assertion rate drops / memory-cache escalations appear" proof is POST-DEPLOY prod telemetry. NOT deployed (V18).
