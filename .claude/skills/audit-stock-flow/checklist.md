...
### S35 — central-PO receive must be a CAS claim (no concurrent double-receive) (V152)
**What**: `receiveCentralStockOrder` must atomically CLAIM the to-receive lineIds into `receivedLineIds` inside a `runTransaction` (`tx.get(centralStockOrderDoc)` → check status/already-claimed → `tx.update(receivedLineIds)`) BEFORE creating any batch. The loser (a concurrent receive that finds the lines already claimed) early-returns without creating batches. Finalize is ALSO a CAS (re-read + MERGE `receivedLineIds`, never overwrite).
**Why**: pre-V152 the idempotency checkpoint read `order.receivedLineIds` via `getCentralStockOrder` (getDoc, OUTSIDE a tx), created batches, and updated `receivedLineIds` only at the END → two concurrent receives (admin double-click "รับสินค้า" / two admins) both saw `receivedLineIds=[]`, both created batches → DOUBLE stock at the central warehouse. This was a DOCUMENTED-deferred AUDIT-V34 gap; confirmed Rule Q L2 `scripts/e2e-stock-order-cancel-central-receive.mjs` C2 (8-unit PO received twice → 16 units / 2 batches BEFORE; 8 / 1 AFTER). Transfer/withdrawal were never vulnerable because their status transition is a CAS-in-tx (S-adjacent). The read-outside-tx class is the same as S32 (V147).
**Where**: `src/lib/backendClient.js` `receiveCentralStockOrder` — the `claimedSet` CAS + loser early-return + `if (!claimedSet.has(lineId)) continue;` loop guard + the finalize merge-CAS.
**How**: grep the function body — MUST contain `const claimedSet = await runTransaction` + `tx.update(oRef, { receivedLineIds:` (claim before build) + `if (claimedSet.size === 0)` + `if (!claimedSet.has(lineId))` + a finalize `runTransaction` that unions `o.receivedLineIds`; NO `KNOWN CONCURRENT-RECEIVE GAP (deferred` comment, NO non-tx `updateDoc(centralStockOrderDoc(...), { items: updatedItems, receivedLineIds:`. Behavior lock: `scripts/e2e-stock-order-cancel-central-receive.mjs` (C2) + source-grep `tests/v152-central-receive-cas.test.js`. Regression: `tests/extended/phase8-adv-orders.test.js` (single-receive + idempotent retry).

### S36 — per-batch expiry edit goes through updateStockBatchExpiry (no qty, no movement, status untouched) (V159)
**What**: Editing a batch/lot's `expiresAt` MUST go through `updateStockBatchExpiry` (`src/lib/backendClient.js`) — ONE `runTransaction` (reads before writes) that updates `batch.expiresAt` + forensic trail (`expiresAtLegacyValue`/`expiresAtEditedAt`/`expiresAtEditedBy`), writes a `be_stock_adjustments` doc with `type:'expiry'` + `movementId:null` (NO `be_stock_movements` write → conservation untouched), and syncs the source order line's `items[].expiresAt` (branch→`be_stock_orders` / central→`be_central_stock_orders` by `locationType`, matched 1:1 by `orderProductId`). It MUST NOT write `batch.status` (the EXPIRED state is derived live via `hasExpired()`, never persisted — S9/S33), and MUST live-resolve `productName` for the audit doc (Rule O).
**Why**: admins mistype expiry easily and must fix it anytime (even after partial use) without re-opening the import order. An expiry edit is NOT a quantity movement → it must not pollute `be_stock_movements` (which the conservation invariants S16 sum) nor carry a qty/linkedMovement. Touching `status` would risk un-/re-setting a derived state; leaving it lets `hasExpired` (live from `expiresAt`) gate FEFO so a corrected future date re-enables the lot immediately. Confirmed Rule Q L2 `scripts/e2e-v159-stock-search-and-expiry.mjs` 28/0 on real prod (batch+forensic+audit+order-sync, no movement, status unchanged, hasExpired flip, 2-concurrent last-write-wins + 2 audit docs + qty intact, idempotency, no-order graceful).
**Where**: `src/lib/backendClient.js` `updateStockBatchExpiry`; the AdjustCreateForm dual-path submit (`src/components/backend/StockAdjustPanel.jsx`); the `type='expiry'` render in the adjust list + `AdjustDetailModal.jsx`.
**How**: grep the `updateStockBatchExpiry` body — MUST contain `runTransaction(`, `type: 'expiry'`, `movementId: null`, `expiresAtLegacyValue`, `_resolveProductNameLive`; MUST NOT contain `stockMovementDoc` or a `status:` write. Anti-regression: any new `type: 'expiry'` adjustment write co-locates with `movementId: null` and lives inside the updateStockBatchExpiry tx. Behavior lock: `scripts/e2e-v159-stock-search-and-expiry.mjs` + source-grep `tests/v159-update-batch-expiry.test.js`.
