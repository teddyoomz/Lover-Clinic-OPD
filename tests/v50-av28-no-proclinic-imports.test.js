...
describe('V50 AV28 — no Firestore reads/writes against deleted collections', () => {
  for (const { re, label } of FORBIDDEN_FIRESTORE_PATHS) {
    it(`AV28.4 — no ${label} in src/ or api/ runtime code`, () => {
      ...
      // V50-followup-2 (2026-05-08) — ALL master_data refs deleted. Was
      // sanctioned: scopedDataLayer.js + backendClient.js. Now: no src/ file
      // references master_data at runtime (institutional-memory comments are
      // stripped before this regex runs via stripComments above).
      expect(violators, `${label} found in: ${violators.join(', ')}`).toEqual([]);
    });
  }
});
