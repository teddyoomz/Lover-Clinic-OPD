...
const COLLECTION_MATRIX = {
  ...
  // baseline / V66 lesson). saveRecallCase carries NO _resolveBranchIdForWrite
  // stamp — global per BSA Rule L (mirror be_staff, be_doctors pattern).
  'be_recall_cases':       { scope: 'global', reason: 'Phase 29.22 — universal recall preset collection (caseName + defaultDays + isHidden). Sub-pill admin UI inside RecallTab. Mirror be_staff/be_doctors universal pattern.' },
  ...
  'be_fb_configs':         { scope: 'global', reason: 'V75 Item 3 (2026-05-16) — per-branch Facebook Page configuration (mirror of be_line_configs). Branch attribution is STRUCTURAL (doc-id === branchId), not via a payload field. Webhook routes by entry[0].id (pageId) → looked up across collection. Classified global so BC2.direct accessor check skips it; per-branch isolation is by doc-id key. Auto-seed banner from legacy clinic_settings/chat_config for นครราชสีมา on first open. AV57 (V75) enforces stamp at webhook write boundary; AV59 (V76) extends to chat_history.' },
