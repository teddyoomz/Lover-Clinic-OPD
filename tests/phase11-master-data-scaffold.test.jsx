...
// Phase 11.1 stub tab IDs — source of truth for iteration.
...
describe('Phase 11.1 — navConfig master section', () => {
  ...
  it('M2 master section has exactly 7 items — 1 sync + 6 CRUD stubs', () => {
    const master = NAV_SECTIONS.find(s => s.id === 'master');
    expect(master.items.length).toBe(7);
...
describe('Phase 11.1 — BackendDashboard routing (deep-link)', () => {
  ...
  it('R5 ?tab=branches renders ComingSoon with "สาขา"', async () => {
    ...
    expect(screen.getByText('Phase 11.6')).toBeInTheDocument();
  });
