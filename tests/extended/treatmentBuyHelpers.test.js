...
describe('buildPurchasedCourseEntry — Phase 12.2b Step 7 (เหมาตามจริง fill-later)', () => {
  ...
  it('BPCE5 เลือกสินค้าตามจริง → placeholder entry with needsPickSelection + availableProducts + empty products[]', () => {
    // Phase 12.2b follow-up (2026-04-24): pick-at-treatment no longer
    // creates fillLater sub-products. Instead the entry is a placeholder
    // — the doctor picks via PickProductsModal which then populates
    // products[] via resolvePickedCourseEntry (see PAP-* tests below).
    const item = {
      id: 'C-PICK', name: 'Pick Course', qty: 0,
      courseType: 'เลือกสินค้าตามจริง',
      products: [
        { id: 'P1', name: 'LipoS', qty: 4, unit: 'เข็ม' },
        { id: 'P2', name: 'Babi', qty: 10, unit: 'ML' },
      ],
    };
    const out = buildPurchasedCourseEntry(item, { now: NOW });
    expect(out.isRealQty).toBe(false);
    expect(out.isPickAtTreatment).toBe(true);
    expect(out.needsPickSelection).toBe(true);
    expect(out.products).toEqual([]); // placeholder — no sub-rows yet
    expect(out.availableProducts).toHaveLength(2);
    expect(out.availableProducts[0].productId).toBe('P1');
    expect(out.availableProducts[0].qty).toBe(4);
    expect(out.availableProducts[0].unit).toBe('เข็ม');
  });
...
describe('resolvePickedCourseEntry — Phase 12.2b pick-at-treatment confirm flow', () => {
  const makePlaceholder = () => ({
    courseId: 'purchased-course-CPICK-123',
    ...
    purchasedItemId: 'CPICK',
    purchasedItemType: 'course',
    availableProducts: [
      { productId: 'P-LIPOS', name: 'LipoS', qty: 4, unit: 'เข็ม', minQty: null, maxQty: null },
      { productId: 'P-BABI', name: 'Babi', qty: 10, unit: 'ML', minQty: null, maxQty: null },
    ],
    products: [],
  });

  it('RPCE1 null/undefined placeholder → passthrough', () => {
    expect(resolvePickedCourseEntry(null, [])).toBeNull();
    expect(resolvePickedCourseEntry(undefined, [])).toBeUndefined();
  });

  it('RPCE2 single pick → 1 product row, needsPickSelection cleared', () => {
    const out = resolvePickedCourseEntry(makePlaceholder(), [
      { productId: 'P-LIPOS', name: 'LipoS', qty: 4, unit: 'เข็ม' },
    ]);
    expect(out.needsPickSelection).toBe(false);
    expect(out.products).toHaveLength(1);
    expect(out.products[0].productId).toBe('P-LIPOS');
    expect(out.products[0].name).toBe('LipoS');
    expect(out.products[0].remaining).toBe('4');
    expect(out.products[0].total).toBe('4');
    expect(out.products[0].unit).toBe('เข็ม');
    expect(out.products[0].fillLater).toBe(false);
  });

  it('RPCE3 multi-pick → multiple product rows in pick order', () => {
    const out = resolvePickedCourseEntry(makePlaceholder(), [
      { productId: 'P-LIPOS', name: 'LipoS', qty: 4, unit: 'เข็ม' },
      { productId: 'P-BABI', name: 'Babi', qty: 2.5, unit: 'ML' },
    ]);
    expect(out.products).toHaveLength(2);
    expect(out.products[0].productId).toBe('P-LIPOS');
    expect(out.products[1].productId).toBe('P-BABI');
    expect(out.products[1].remaining).toBe('2.5');
  });

  it('RPCE4 invalid picks (missing productId / zero qty) filtered out', () => {
    const out = resolvePickedCourseEntry(makePlaceholder(), [
      { productId: '', name: 'Bad', qty: 1, unit: 'x' },
      { productId: 'P-LIPOS', name: 'LipoS', qty: 0, unit: 'เข็ม' },
      { productId: 'P-LIPOS', name: 'LipoS', qty: 4, unit: 'เข็ม' },
    ]);
    expect(out.products).toHaveLength(1);
    expect(out.products[0].productId).toBe('P-LIPOS');
  ...
  it('RPCE5 preserves placeholder metadata (courseId, purchasedItemId, isAddon)', () => {
    const p = makePlaceholder();
    const out = resolvePickedCourseEntry(p, [
      { productId: 'P-LIPOS', name: 'LipoS', qty: 4, unit: 'เข็ม' },
    ]);
    expect(out.courseId).toBe(p.courseId);
    expect(out.courseName).toBe(p.courseName);
    expect(out.purchasedItemId).toBe(p.purchasedItemId);
    expect(out.isAddon).toBe(true);
    expect(out.isPickAtTreatment).toBe(true); // type still set for audit
  });

  it('RPCE6 empty picks → empty products[] + needsPickSelection still cleared', () => {
    const out = resolvePickedCourseEntry(makePlaceholder(), []);
    expect(out.products).toEqual([]);
    expect(out.needsPickSelection).toBe(false);
  });

  it('RPCE7 generated rowId deterministic by productId', () => {
    const out = resolvePickedCourseEntry(makePlaceholder(), [
      { productId: 'P-LIPOS', name: 'LipoS', qty: 4, unit: 'เข็ม' },
    ]);
    expect(out.products[0].rowId).toContain('P-LIPOS');
  });

  it('RPCE8 qty coerces to string (treatment renderer needs string format)', () => {
    const out = resolvePickedCourseEntry(makePlaceholder(), [
      { productId: 'P-LIPOS', name: 'LipoS', qty: 4, unit: 'เข็ม' },
    ]);
    expect(typeof out.products[0].remaining).toBe('string');
    expect(typeof out.products[0].total).toBe('string');
  });
});
