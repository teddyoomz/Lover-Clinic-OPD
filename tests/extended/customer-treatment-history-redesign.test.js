...
describe('H3: header CTAs (พิมพ์เอกสาร / บันทึกการรักษา / ดูไทม์ไลน์)', () => {
  ...
  it('H3.3: ดูไทม์ไลน์ button wired to local showTimeline state (Phase 14.7.E live)', () => {
    expect(SRC).toMatch(/data-testid="show-timeline-btn"/);
    expect(SRC).toMatch(/ดูไทม์ไลน์/);
    expect(SRC).toMatch(/setShowTimeline\(true\)/);
    // No longer disabled — modal is live.
    expect(SRC).not.toMatch(/data-testid="show-timeline-btn"[\s\S]{0,400}disabled=/);
  });
