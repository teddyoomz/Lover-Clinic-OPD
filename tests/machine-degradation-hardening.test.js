...
describe('F — TFP fast-paint pre-stage (≤5s-เมื่อเน็ตโอเค directive, 2026-07-20)', () => {
  ...
  it('F7 hunt-R1 money-action gate: optionsEnriched blocks save+buy during the fast-paint window', () => {
    ...
    // handleSubmit guard (create-mode; hunt-R2: vitals EXEMPT — its path skips
    // every money block, and the vitals station is fast-paint's audience)
    expect(TFP).toMatch(/if \(!isEdit && !optionsEnriched && saveMode !== 'vitals'\) \{\s*setError\('กำลังโหลดข้อมูลคอร์ส/);
    expect(TFP).toMatch(/if \(!isEdit && !optionsEnriched\) \{\s*setError\('กำลังโหลดรายการคอร์ส/);
    // doctor + main staff save disabled on the gate; vitals button is NOT
    const disables = TFP.match(/disabled=\{saving \|\| \(!isEdit && !optionsEnriched\)\}/g) || [];
    expect(disables.length).toBeGreaterThanOrEqual(2);
    const vitalsBtnIdx = TFP.indexOf('data-testid="tfp-vitals-save-btn"');
    expect(TFP.slice(vitalsBtnIdx - 300, vitalsBtnIdx + 100)).not.toMatch(/optionsEnriched/);
    expect(TFP).toMatch(/canAddNewItems && \(isEdit \|\| optionsEnriched\) &&/);
  });

  it('F8 hunt-R2 painted-form enrichment escape: stuck banner + retry when enrichment stalls', () => {
    // After fast-paint the full-screen escape card can't render (under
    // `if (loading)`) — a stalled enrichment would strand greyed money buttons
    // forever. The stuckTimer stays armed (NOT cleared at fast-paint) and the
    // painted form renders an inline banner with the same reconnect+retry.
    expect(TFP).toMatch(/data-testid="tfp-enrich-stuck-banner"/);
    expect(TFP).toMatch(/\{!isEdit && !optionsEnriched && loadStuck && \(/);
    const bannerIdx = TFP.indexOf('tfp-enrich-retry-btn');
    expect(bannerIdx).toBeGreaterThan(-1);
    const w = TFP.slice(bannerIdx, bannerIdx + 900);
    expect(w).toMatch(/loadRunSeqRef\.current\+\+/);       // invalidate hung run FIRST (R1-#1 contract)
    expect(w).toMatch(/reconnectFirestore\(\)/);
    expect(w).toMatch(/setLoadRetryNonce\(n => n \+ 1\)/);
    // fast-paint must NOT clear the escape timers (the banner depends on stuckTimer)
    const fpStart = TFP.indexOf('AV212 FAST-PAINT');
    const fpEnd = TFP.indexOf('(async () => {', fpStart);
