...
describe('C1-C5 — pill + menu + insert', () => {
  ...
  it('C2 เปิดเมนู → fetch ครั้งเดียว + builtin แถวแรก + badge บังคับ + ไม่มี ✎/🗑 บน builtin', async () => {
    render(<OpdNoteTemplateMenu isDark onInsert={() => {}} />);
    await openMenu();
    ...
    // re-open → ไม่ fetch ซ้ำ (lazy once)
    fireEvent.click(screen.getByTestId('opd-template-trigger')); // close
    fireEvent.click(screen.getByTestId('opd-template-trigger')); // open
    expect(mockList).toHaveBeenCalledTimes(1);
  });
